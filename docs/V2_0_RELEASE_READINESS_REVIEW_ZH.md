...
- **CI 全部通过** ✔：第 11 步 `Build release`、第 12/14 步「干净安装」「未配置安装」、**第 16 步「V1.5 升级、二次升级、卸载」** 以及产物上传均为 `success`（run `34755944510`，2026-09-13）。第 16 步曾失败而本机通过，真因**不是产品缺陷**：V1.5 安装器的 `[Run]` 没有 `skipifsilent`，静默安装也会启动 V1.5 应用，该实例把默认配置抢先写进中央用户目录（实测比候选版安装器初始化早 0.7 秒），而候选版安装器**按设计不覆盖已有用户配置**，于是跳过迁移并 `exit 0`、保留了那份默认配置。已在测试侧修正：等它启动并停掉（CI 日志可见 `the previous release started itself after a silent install; stopping it: VibeFlow#5016[…\previous-app\VibeFlow.exe]`）、把 fixture 写到真实用户配置所在位置、并用 SHA-256 自证后才交给安装器（详见 `docs/V2_0_PROGRESS.md`）。

### 7.1 正式版落地（同日）

- **已发布** ✔：`v2.0.0`（`prerelease=false`，Latest），沿用候选版 3 已核验的同一份出货构建（三个附件哈希一致）。§1 里"作为正式稳定版发布：暂不可以"的判断基于当时的六项前提；发布方在**知情**的前提下以"沿用已核验构建 + 明文披露未签名与未验证项"的方式发布了正式版，因此本文件随之记录为**已发布**，而**不是**把那些前提改成已满足。
- **仍然未满足的正式版前提**（不要在别处写成已通过）✗：代码签名、一台**没有 VB-CABLE** 的机器首启、出货产物上的**真机语音**、**高 DPI 目视**、已发布安装包内文档的截图快照（`07/08` 仍是 V1.5 画面）。
- **已同步** ✔：`frozen-parameters.json` 的 tag/releaseName/commit、发布正文模板（=线上正文逐字一致）、以及所有当前版本口径与下载入口（README、快速开始、教程、功能看板、更新说明、安装器说明、FAQ、版本文件）。
