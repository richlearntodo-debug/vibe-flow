...
选择一个语音工具（微信输入法 / 八哥说 / 讯飞语音输入法 / 其他语音工具），并按工具自己的说明设置快捷键和触发方式。Vibe Flow 两边的快捷键必须完全一致。选择讯飞语音输入法时，请在讯飞输入法「设置 → 语音」把语音快捷键设为 `Ctrl + Shift + Alt + [`，并选择「长按说话」。
...
当前候选版只提供四个可选语音工具，顺序与「语音」页一致：

1. **微信输入法**（默认，`Ctrl + Win`，单击切换）；
2. **八哥说**（网易，右 Alt，单击切换）；
3. **讯飞语音输入法**（本候选版新增支持，`Ctrl + Shift + Alt + [`，按住触发）；
4. **其他语音工具**（自定义，默认右 Shift，单击切换）。
...
1. 在「语音」页把「转写工具」选成 **八哥说**；默认快捷键会填 **右 Alt**（`rightalt`）。
2. **在八哥说自己的设置里确认它的启动/结束快捷键就是右 Alt**；如果它在客户端里用的是别的组合，请把语音页的快捷键改成**完全一致**的那个（应用与工具两边不一致时，按键按下不会启动听写）。
3. 保持「单击切换」触发方式（默认）。先在键盘上按一次右 Alt 确认八哥说能启动与结束，再用遥控器录音键测试。
4. 先不要依赖自动粘贴：八哥说是否把文字直接写进输入框**尚未在本机实测**，所以言灵**不会**为它派发 `Ctrl+V`。如果实测发现它其实把结果放进剪贴板，需要在语音页改用「其他语音工具」并手动粘贴，或等待我们对它的投递方式做实测后适配。
5. 言灵只负责派发触发动作与维持录音，**不读取**八哥说的转写内容。

### 讯飞语音输入法

讯飞语音输入法（iFlytek，本机安装在 `D:\iFlyIME\3.0.1750`）是本候选版新增支持的工具，默认快捷键是 `Ctrl + Shift + Alt + [`，触发方式是**按住触发**。

1. 在「语音」页把「转写工具」选成 **讯飞语音输入法**；默认快捷键会填 `Ctrl + Shift + Alt + [`，触发方式默认是「按住触发」。
2. **在讯飞输入法「设置 → 语音」里确认它的语音快捷键就是 `Ctrl + Shift + Alt + [`**（这是讯飞安装后自己写入的默认值）。如果它在客户端里用的是别的组合，请把语音页的快捷键改成**完全一致**的那个。
