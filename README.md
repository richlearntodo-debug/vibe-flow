...
5. 在“语音”页选择语音工具。需要直接写入输入框时，优先测试 Windows 语音输入（`Win+H`）；微信输入法的语音面板行为由第三方工具决定，可能把文字放到剪贴板。
...
- 微信输入法可能按第三方工具自己的方式把识别结果放入剪贴板；Vibe Flow 不保证所有第三方输入法都能直写。需要完全直写路径时优先选择 Windows 语音输入，并以实际测试结果为准。
