...
4. 选择语音工具（微信输入法 `Ctrl + Win` 单击切换 / 网易八哥说 右 Alt 单击切换 / 讯飞语音输入法 `F6` 按住触发 / 搜狗输入法 右 Ctrl 按住触发 / 豆包输入法 `Alt + 空格` 按住触发、支持但未验证 / 其他语音工具 右 Shift 单击切换），设置与客户端一致的快捷键，并完成一次真实听写；
...
需要排查“文字进剪贴板”时，改用讯飞语音输入法做对照：在讯飞输入法「设置 → 语音」把语音快捷键设为 `F6`（讯飞里唯一可设置的语音快捷键），并选择「长按说话」，与言灵保存的快捷键和触发方式保持一致。改用搜狗输入法时，请确认它自己的语音/按键设置里语音输入快捷键是右 Ctrl 且为按住说话，两边必须相等；豆包输入法是支持但未验证的路径（其语音面板可能忽略模拟按键、麦克风列表可能没有 `CABLE Output`），不要拿它当对照基准。微信输入法部分版本会把识别结果放入剪贴板，这是第三方行为；Vibe Flow 不读取转写文字，也不保证所有微信输入法目标都能直写。
