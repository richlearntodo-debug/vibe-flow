...
- Trigger-only mode (no VB-CABLE) now names the tools the host can actually wake by itself: 八哥说, 讯飞语音输入法 and a custom tool. The copy that told the user to switch to Windows 语音输入 is gone with the tool.
