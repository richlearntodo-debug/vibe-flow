...
4. 选择语音工具（微信输入法 / 八哥说 / 讯飞语音输入法 / 其他语音工具），设置与客户端一致的快捷键，并完成一次真实听写；
...
需要排查“文字进剪贴板”时，改用讯飞语音输入法做对照：在讯飞输入法「设置 → 语音」把语音快捷键设为 `Ctrl + Shift + Alt + [`（讯飞安装后的默认值），并选择「长按说话」，与言灵保存的快捷键和触发方式保持一致。微信输入法部分版本会把识别结果放入剪贴板，这是第三方行为；Vibe Flow 不读取转写文字，也不保证所有微信输入法目标都能直写。
