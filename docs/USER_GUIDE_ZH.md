# 言灵 · Vibe Flow Remote v1.2.0 使用教程

这份教程面向第一次使用 RC003 蓝牙语音遥控器、没有编程基础的 Windows 用户。跟随应用内五步向导即可完成配置。

> **推荐操作是单击开始、再次单击结束。** 开始后不需要持续按住录音键，适合长提示词和连续口述。

> [!NOTE]
> 连续听写单次有 30 分钟软件安全保护。兼容的“按住说话”模式仍受 RC003 约 60 秒物理长按边界限制。
...
升级不会覆盖现有 `vibe-mic-config.json`，因此转写工具、快捷键、开机启动和语音模式会保留。升级后在“偏好设置”确认版本为 `1.2.0`，再到“连接与自检”运行一次检查。旧用户希望使用长时听写时，可在“语音听写”选择“连续听写（推荐）”。
...
| 工具 | 推荐快捷键 | 触发方式 | 说明 |
| --- | --- | --- | --- |
| 微信输入法 | `Ctrl + Win + Shift` | AI 整理（单击切换） | 已完成 RC003 15 分钟长时听写、AI 整理与完整句尾真机验证 |
...
4. 录音方式选择“连续听写（推荐）”。单击遥控器录音键后，言灵会在音频通道就绪时第一次轻触快捷键；再次单击结束后，先排空全部尾音，再第二次轻触快捷键并恢复原来的 Windows 麦克风。
5. 不要选择微信工具栏或状态栏入口。该路径可能进入“已复制文本”模式，而不是直接写回原输入框。

`Ctrl + Win` + “按住触发”仍可用于旧版微信输入法，但它不会启用 AI 整理。本机对同一段 RC003 音频的 A/B 验证中，该兼容配置遗漏了句尾，推荐配置保留了完整内容并完成结构化整理。
...
![完成首次连续听写](images/00-setup-5-dictation.png)

1. 点击空白测试输入框，确保光标闪烁。
2. **单击录音键并松开**，等待页面显示“遥控器音频正在到达”。
...
2. 单击录音键并松开。首页先显示青色“正在恢复遥控器音频”。
3. 看到紫色“遥控器音频正在到达”后自然说话，无需持续按住。
4. 说完后再次单击录音键。
5. 青色表示正在排空并交给工具整理；绿色表示文字已经完成回填。结束提示音只在第 4 步播放，不会完成后再响一次。

连续模式的三项核心保证：

- **真实音频反馈**：只有收到 RC003 音频包后才显示正在听写，不能用计时假装正在收音。
- **稳定续流**：每 8 秒只向当前精确会话续租，断流会有界恢复，失败会明确停止并提示。
- **有界收尾**：结束后先排空尾音，再关闭一次转写会话并恢复原来的麦克风，不自动打开第二个麦克风。

当前发布参数已完成 124 秒、6 分钟和 15 分 22 秒 RC003 真机回归，最长一轮真实音频覆盖率 `99.6%`、`MIC_EXTEND 114/114`、蓝牙与 VB-CABLE 丢包均为 `0`。单次设置 30 分钟软件安全保护，不宣传绝对无限时长。

习惯按下说话、松开结束的用户可在“语音听写”选择“按住说话（兼容）”。该模式已验证松开后 80 ms 内自然停止且不会弹出第二个麦克风，但受 RC003 固件约 60 秒物理长按边界限制。
...
2. 单击录音键并松开，等待遥控器麦克风出现紫色光效并开始计时。
3. 持续说话 75 秒以上，再次单击结束；全程只出现一次微信麦克风。
...
- **录音方式**：新安装默认“连续听写”；单击开始、再次单击结束，单次有 30 分钟软件保护。升级用户保留原选择。“按住说话”作为约 60 秒的兼容模式保留。
...
| 遥控器按键 | 默认行为 | 可配置 |
| --- | --- | --- |
| 录音键 | 连续模式单击开始、再次单击结束；兼容模式按住开始、松开结束 | 固定 |
...
“最近一次听写”会显示逻辑总时长、真实音频时长、覆盖率、输出电平、最大包间隔、续租、丢包和麦克风恢复状态。连续模式若只在计时却没有真实音频，自检会直接标红。
...
| 连续听写在约 60 秒结束 | 录音方式与真实音频覆盖 | 确认选择“连续听写（推荐）”；运行自检并查看主机音频流与续租，不要持续按住录音键 |
| 按住说话约 60 秒自动结束 | RC003 硬件边界 | 这是兼容模式的固件行为；长内容切换为连续听写，不要调节增益或蓝牙参数 |
| 结束后再次出现麦克风 | 录音方式和会话代次 | 确认只再次单击一次结束，配置 schema 应为 20；重启言灵后复测，仍出现则导出诊断 |
...
RC003 会在一次物理长按约 60 秒时同时发送 `F5 UP` 和 ATVV `stream_stop`。即使用户仍在按住，Windows 看到的状态也已经是“松开”。言灵因此不会在按住模式中自动重开，避免结束后弹出第二个麦克风。需要更长内容时请切换为连续听写：单击开始并松开，完成后再单击一次结束。
...
发布前必须逐张检查：版本号、单击开始/结束文案、录音模式、文字截断、控件重叠、真实连接状态和隐私信息。
