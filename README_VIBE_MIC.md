...
Supported providers, in the order the Voice page lists them, are WeChat Input Method (default: `Ctrl + Win`, toggle, `80 ms`), NetEase Bage (网易八哥说: Right Alt, toggle), iFlytek Voice Input (讯飞语音输入法: `F6`, hold), Sogou Input Method (搜狗输入法: Right Ctrl, hold), Doubao Input Method (豆包输入法: `Alt + Space`, hold), and one custom slot (其他语音工具: Right Shift, toggle). iFlytek is configured inside 讯飞输入法 → 设置 → 语音, where its own 语音快捷键 must equal the stored `F6` and 长按说话 matches the app's hold trigger. Sogou is configured inside 搜狗's own 语音/按键 settings, where the voice shortcut must equal the stored Right Ctrl and 按住说话 matches the hold trigger. Doubao was retired in an earlier round, came back at the user's request, and is supported but unverified: its voice panel may ignore synthetic keystrokes, and its microphone list may not offer the virtual cable `CABLE Output` — its stored `Alt + Space` hold trigger must still be matched inside Doubao's own settings. Typeless and Windows Voice Typing are no longer selectable: a stored value for either is migrated to WeChat Input Method and reported to the user by name. In trigger-only mode (no VB-CABLE, so no capture process runs) the host can wake NetEase Bage, iFlytek Voice Input, Sogou Input Method and a custom tool by itself; WeChat Input Method needs its own voice panel and therefore still requires VB-CABLE.
