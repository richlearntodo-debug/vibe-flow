...
- **第 11 步「Build release」已通过** ✔（曾经失败是因为 `tools\VBCABLE_Driver_Pack45.zip` 被 gitignore：现在恢复脚本尽力下载并校验哈希，发布链把它当**可选**，CI 实测在缺包时也能 exit 0）。
- **第 12、14 步「干净安装」「未配置安装」已通过** ✔（曾经秒退是因为 `Test-ReleaseLifecycle.ps1` 无 BOM，在英文 runner 上无法解析；现已全量补 BOM 并加门禁）。
- **第 16 步「V1.5 升级、二次升级、卸载」已通过** ✔（CI run `34755944510`，2026-09-13 20:07，整条 workflow `success`）。曾经失败的原因已由日志时间戳定案，且**不是产品缺陷**：V1.5 安装器的 `[Run]` 没有 `skipifsilent`，**静默安装也会启动 V1.5 应用**，该实例按 V1.5 的规矩把默认配置写进**中央用户目录**（实测：目标文件写入时间比候选版安装器初始化早 0.7 秒）；候选版安装器**永不覆盖已有用户配置**（设计如此），于是跳过迁移、`exit 0`、保留了那份默认配置，测试便拿它去和 fixture 比对。本机之所以通过，是因为本机有在跑的候选版实例持有 `Local\VibeMic` 互斥体，V1.5 应用启动后立即退出 —— 是并发时序，不是环境玄学。修正在测试侧：等它启动并停掉（CI 日志：`the previous release started itself after a silent install; stopping it: VibeFlow#5016[…\previous-app\VibeFlow.exe]` ✔）→ 把 fixture 写到**真实用户配置所在位置**（中央目录）→ 交给安装器前用 SHA-256 自证未被改写；安装目录那份旧配置的迁移路径由 `scripts/tests/Test-InstallerConfigMigration.ps1` 覆盖 ✔。
- 由此，**三条生命周期路径在本机与 CI 上均已通过** ✔；安装器另加日志（上一安装目录、旧文件是否存在、迁移助手退出码），测试传 `/LOG` 并在投影不一致时打印这些行与 fixture 自证结果 ✔。
...
| 发布标签 | **`v2.0.0`** ✔（2026-09-13 正式版，沿用候选版 3 已核验的同一份出货构建；候选版发布页 `v2.0.0-candidate.3` 保留用于复现历史测试） |
...
- **CI 上的同一序列**：三条路径全部通过 ✔（2026-09-13 的 run `34755944510`）。第 3 条（V1.5 升级）曾在 CI 上失败而本机通过，真因是 V1.5 安装器静默安装时也会启动 V1.5 应用、该实例抢先写了一份默认配置到中央用户目录（详见 §8），已在测试侧修正并以 SHA-256 自证 ✔。
- 仍未验证 ✗（如实记录 ✔）：**没有 VB-CABLE 的机器首启**、**代码签名**、发布产物上的**真机语音**（用户已在候选版上确认核心功能 ✔，但该结论来自测试版构建 ✔），以及**已发布安装包内文档的截图快照**（`07/08` 两张对话框图仍是 V1.5 画面，仓库与下次候选包已更新 ✔）。
