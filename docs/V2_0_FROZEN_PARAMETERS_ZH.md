...
- **第 16 步「V1.5 升级、二次升级、卸载」仍失败** ✗：CI 报「Install or upgrade did not preserve provider, mappings, Profiles, Smart Profile state, theme, or startup preferences」，**本机同一序列通过** ✔，所以这是环境相关而不是已知的产品缺陷 —— 迁移助手本身已排除（按安装器真实引用方式运行 `VibeFlow.exe --installer-config-migrate`，对同一份含已退役 `typeless` 的 fixture **exit 0 且逐字复制** ✔）。已加日志（安装器记录注册表给出的上一安装目录、迁移时旧文件是否存在、迁移助手退出码；测试传 `/LOG` 并在不一致时打印这些行），下一次 CI 运行据此定位。**尚未解决，不写成已修复。**
...
- **CI 上的同一序列**：第 1、2 条已通过 ✔（2026-09-13 的 tag 运行）；第 3 条（V1.5 升级）在 CI 上仍失败 ✗（详见 §8），因此"生命周期验收"只在本机成立，不能写成 CI 已绿。
