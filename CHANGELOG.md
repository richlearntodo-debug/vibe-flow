...
- Fixed that failure. The installer log showed the right previous install directory, the legacy file present and the helper exiting 0, yet the destination held defaults — because the **published V1.5 installer's `[Run]` entry has no `skipifsilent`**, so a silent install of the previous release still starts it. That instance registers the configuration in its own directory and writes it back, replacing the fixture the lifecycle test had just written with V1.5's own defaults; the V2 installer only checks that the file exists, so it migrated the replaced file. The test now stops every Vibe Link process before handing the fixture over and proves the fixture's SHA-256 is still the text it wrote (repeated, so a slow start is caught too), failing loudly if something keeps rewriting it, and it prints the fixture's state when the projection differs. This also explains why the same sequence passed on this machine: a running V2 instance holds the `Local\VibeMic` mutex, so the V1.5 app exits immediately instead of overwriting anything.
