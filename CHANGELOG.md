...
## 2.0.0 - 2026-09-13 (正式版)

- Published Vibe Link V2.0.0 as the formal release, reusing the verified candidate.3 payload: `VibeFlow-Setup.exe`, `Vibe-Flow-Windows-x64.zip` and `SHA256SUMS.txt` are byte-identical to candidate.3 (same SHA-256), so the verification done on candidate.3 carries over and the release notes say so. The build is still **unsigned**; the installer lifecycle (clean install, unconfigured install, V1.5 upgrade, second upgrade, uninstall) passes locally and in CI, and the hardware, DPI and no-VB-CABLE checks that have not been done stay listed in the known-limitations document instead of being implied.
- Synced the frozen record and the release-surface documents to the formal release. `frozen-parameters.json` now names tag `v2.0.0` and release name `Vibe Link V2.0.0 正式版` (it is what the publishing script reads, so it previously pointed a republish at the candidate page). The GitHub release body template `docs/GITHUB_RELEASE_BODY_ZH.md` is now the published formal body verbatim, so the next publish cannot regress the screenshot and document links to `v2.0.0-candidate.3`, and the current-version download links and version statements in the README, quick start, user guide, feature board, release notes, installer guide, FAQ, `VIBE_MIC_VERSION.md` and `README_VIBE_MIC.md` point at `/v2.0.0/` and describe V2.0.0 as the formal release.
- Kept the candidate.3 references that are historical record — its own CHANGELOG section, the progress log entries and the candidate release page — and marked the candidate page as the archive used to reproduce the historical test runs, so the history stays readable while the "current version" statements are true.

## 2.0.0 candidate.3 - 2026-09-13

- Cut the selectable voice tools to three, at the user's request, and removed everything that kept the withdrawn ones alive: 微信输入法 (default, `Ctrl + Win`, single tap), 网易八哥说 (Right Alt, single tap) and 其他语音工具 (`rightshift`, single tap) are the whole list. 讯飞语音输入法, 搜狗输入法 and 豆包输入法 joined Typeless and Windows 语音输入 in retirement: they are gone from the dropdowns, the key normaliser, the default shortcuts and triggers, the process probes, the launcher lookups and the user-facing copy, and a stored configuration carrying any one of them migrates to 微信输入法 with the stable defaults and a one-time notice. The host self-test walks thirteen historical values through that migration, built from character arrays so the withdrawn names do not survive in the runtime surface, and a repository gate fails if one reappears. Every selectable tool still has a default shortcut no other tool has.
- Fixed launch-at-startup writing and reading different Run value names. `SetLaunchAtStartup` wrote `Vibe Flow` while the delete and read-back paths used `Vibe Link`, so the registration could not be turned off, every start logged a repair, and the reconcile result stayed `applied=false`. The name is one shared constant now — the same literal the installer uses — the legacy names are cleared on both paths, and the repository gate fails if any read or write uses a literal of its own. Verified on the machine: the reconcile line now reports `applied=True`.
- Corrected the release surface. The GitHub release body — copied verbatim by the release build into `release/RELEASE_BODY_v2.0.0.md` — no longer tells users to pick a withdrawn voice tool: it names 微信输入法 / 网易八哥说 / 其他语音工具 only, carries the `v2.0.0-candidate.3` download, screenshot and tag links, and states plainly that the installer is **unsigned** with the SmartScreen steps (未知发布者 → 更多信息 → 仍要运行). The release-body title, the README, the version document, the release checklist and every other current-version download link move to `v2.0.0-candidate.3`, and the user-visible product name is **Vibe Link**.
- Corrected the current-version documents that disagreed with the shipped build: the page count in the hardware matrix and the automated test report (五页 → 六页), the user guide's long-press duration (650 ms → the code's 600 ms), the known-limitations entry on the power key (a normal mapped key whose three layers are the DeepSeek usage page, Bilibili and the task switcher, and which never starts a recording), the stale Live HUD lines in `docs/V2_0_PROGRESS.md` and their references to a layout screenshot that no longer exists, and the typo 应内选择 → 应用内选择.
- Published `v2.0.0-candidate.3` and corrected two defects the publication itself exposed. Both had the same shape: a BOM-less script read by Windows PowerShell 5.1 through the ANSI code page. The first attempt sent the release **name** as mojibake, and the body was rejected outright; the name now comes from `frozen-parameters.json` instead of a literal in the publishing script, and the body is decoded from UTF-8 bytes rather than through the console code page. The published page also showed the same screenshot twice — its last cell pointed at `03-shortcuts-screenshot.png`, a byte-identical twin of `03-shortcuts.png` — and now illustrates the three selectable voice tools with the tool dropdown, checked against the copy of that image the page itself serves.
- Guarded both defects with repository gates: every screenshot the release body names must exist in `docs/images`, no screenshot may repeat inside one section, and every label `capture-ui-screenshots.ps1` matches by hand must still be rendered by the product source. That last gate exists because `07-shortcut-actions.png` and `08-shortcut-recorder.png` had stayed on the V1.5 dialogs for ten days: the action box they were captured from was renamed, and the capture skipped them silently instead of failing. It refuses now, `-DialogsOnly` recaptures just the dialogs, and all three dialog screenshots were retaken from the shipping build (the picker's title now reads 配置 上键短按).
- Made the CI V1.5-upgrade failure name its cause instead of only reporting that something was not preserved. The installer now logs the previous install directory it was handed and the migration helper's exit code, and the lifecycle test passes `/LOG` and prints those lines when the projection differs. The helper itself is not implicated: run the way the installer runs it, `VibeFlow.exe --installer-config-migrate` copies this fixture verbatim, retired provider value included, and exits 0.
- Fixed that failure. The installer log showed the right previous install directory, the legacy file present and the helper exiting 0, so the migration was not at fault: the destination already held a configuration. The **published V1.5 installer's `[Run]` entry has no `skipifsilent`**, so a silent install of the previous release still starts it, and that instance writes its own default configuration into the central user-data directory — measured on CI, 0.7 s *before* the candidate installer initialised. The candidate installer deliberately never overwrites an existing user configuration, so it skipped the migration, reported success and preserved that default, which the test then compared against its fixture. The lifecycle test now stops the previous release's launch before writing the fixture, and writes the fixture where a real V1.5 user's settings live (the central directory, not the installation directory), proving the fixture's SHA-256 is still the text it wrote before handing it over and failing loudly if something keeps rewriting it. The installation-directory migration that an older release needed stays covered by `scripts/tests/Test-InstallerConfigMigration.ps1`. This also explains why the same sequence passed on this machine: a running V2 instance holds the `Local\VibeMic` mutex, so the V1.5 application exits immediately instead of writing anything. Verified end to end: CI run `34755944510` is green on all three lifecycle paths, and its log shows the previous release being caught and stopped (`the previous release started itself after a silent install; stopping it: VibeFlow#5016[…\previous-app\VibeFlow.exe]`) before the fixture was handed to the installer.

## 2.0.0 candidate.2 - 2026-09-12

*Historical record: the entries below describe the candidate.2 build as it was made. 讯飞语音输入法 and 搜狗输入法 were added and then withdrawn inside that same candidate, and neither they nor 豆包输入法 are selectable in candidate.3.*
...
- Removed Typeless and Windows 语音输入 as selectable voice tools, on the user's verdict after testing both. Selecting Typeless actually triggered 八哥说, because both tools had been given Right Alt as their default; Windows dictation started and stopped repeatedly with the two cue sounds running together, because Win+H is a single-tap toggle and the app was holding it down. Both values are now retired: they are gone from every dropdown, the key normaliser, the default shortcut and trigger, the process match and the status copy, and a stored configuration holding either one is migrated to 微信输入法 with a toast that names the tool it replaced. The host self-test asserts both values stay retired and that the migration records which one it saw.
- Added 讯飞语音输入法 as a supported voice tool. It is installed on this machine (D:\iFlyIME\3.0.1750) and its voice bar can only be bound to F6 — its own installer had written `Ctrl + Shift + Alt + [` into `HKCU\Software\iFly Info Tek\iFlyIME -> iFlyImeVoiceShiftHotKey`, but the user found that the client's settings allow F6 and nothing else — so `f6` is the shortcut the app stores, with 按住触发 as the trigger because 讯飞's voice bar is documented as 长按说话 / 松手结束. The tool is wired through the key normaliser (including its Chinese label), the display name and summary, the default shortcut, trigger and startup delay, the process match that decides whether its client is running, its launcher (read from its uninstall record, since it installs outside the user profile), and the label/index mapping the combo boxes rely on.
- Fixed two components pressing the same tool shortcut. The frozen capture presses a tool's shortcut itself in a full session and the host also pressed it for some tools, so a toggle was pressed twice and a hold overlapped a tap: the user saw this as tools that started and stopped by themselves. Exactly one component owns the press now — the capture whenever it can parse the shortcut, the host only for a shortcut outside the capture's parser (讯飞's `Ctrl + Shift + Alt + [`, because that parser knows only letters, digits, F1-F24 and a short list of named keys) or in trigger-only mode where no capture runs. The host can therefore press punctuation keys, and `scripts/validate.js` compares the host's mirror of the capture's key list with the capture's own list word for word, because the capture is frozen and a shortcut added on one side alone would silently hand the press to nobody. 讯飞's F6 is inside the capture's reach, so for 讯飞 the capture sends the press and the host stays out of it; the punctuation support stays as a capability and is pinned by the host self-test.
- Trigger-only mode (no VB-CABLE) now names the tools the host can actually wake by itself: 网易八哥说, 讯飞语音输入法, 搜狗输入法 and a custom tool. The copy that told the user to switch to Windows 语音输入 is gone with the tool.
- Added 搜狗输入法 as a supported voice tool. The user's own 搜狗 configuration binds its voice input to Right Ctrl and is spoken into by holding it, so the app stores `rightctrl` with 按住触发, locates the client through its uninstall record (it installs outside the user profile, at D:\SogouInput\<version>), recognises its processes (SGTool, SGOUSmartAssistant, sogou_voice_assistant, SogouCloud) and carries its TSF identity, which was read off this machine so the self-check page names it instead of 未知输入法.
- 豆包输入法 is selectable again, at the user's request. It had been retired in an earlier round, so it is removed from the retired set — the retired predicate is now exactly Typeless and Windows 语音输入 — and it is wired like every other shortcut-driven tool with the shortcut measured when it was first adapted, Alt + Space, held while speaking. Two risks recorded in that earlier round are still unverified and the docs say so: its voice panel may ignore simulated keystrokes, and its microphone list may not offer the virtual cable CABLE Output.
- 网易八哥说 is the display name of the tool that was shown as 八哥说, because the user asked for the vendor to be spelled out. Shortcut, trigger and behaviour are unchanged; the HUD, the context deck, the voice page, all four combo boxes and every user-facing sentence follow the new name.
- Every selectable voice tool now has to have a default shortcut no other tool has, and the host self-test proves it by comparing all six pairs instead of listing one pair. 搜狗 (Right Ctrl), 豆包 (Alt + Space) and 讯飞 (F6) join 微信输入法 (Ctrl + Win), 网易八哥说 (Right Alt) and the custom slot (rightshift).
- Withdrew 搜狗输入法, 讯飞输入法 and 豆包输入法 at the user's request, which leaves 微信输入法, 网易八哥说 and the custom slot as the whole list. 搜狗 was added earlier in this same candidate and the user reported that it does not work on their machine; 讯飞 and 豆包 were withdrawn together with it ("这 3 个我们暂时不需要支持"). Everything that existed for them is gone: the key normaliser, all four combo boxes, the display names, summaries and setup instructions, the default shortcut, trigger and startup delay, the label/index mapping, the process match, the running-client probe, the launcher lookup added for them (and the helper it used, now unreachable), the three input-method identities in the engine catalogue, the guidance copy, the host self-test and the repository gates. The engine catalogue now names only 微信输入法 and 微软拼音, so a keyboard owned by one of the withdrawn input methods reads as an unknown engine rather than as a supported tool.
- Made the retired-provider migration a catch-all instead of a list. `IsKnownProviderValue` holds only the three supported values and `IsRetiredProviderValue` retires anything else, so a configuration written by any earlier build — Typeless, Windows 语音输入, 讯飞, 搜狗, 豆包 or a value from a future one — always migrates to 微信输入法 with the stable defaults and a one-time notice, and can never behave like a supported tool. The host self-test walks thirteen historical values through that migration, built from character arrays so the withdrawn names do not survive in the runtime surface, and a repository gate scans the host, the input-method detector and the workflow cards for those names token by token. A negative control that removed the catch-all made the self-test fail, and restoring it passed.
