...
- **发布面修正**：安装包**未签名**，Windows 会显示「未知发布者」或 SmartScreen 提示（处理步骤见下文「安装版和便携版的区别」）；完整性校验以随包发布的 `SHA256SUMS.txt` 为准，本文不再抄写会随构建过期的资产哈希。
- **兼容性与一致性修正**：六个页面固定为 首页 · 语音 · 快捷键 · 工作流 · 自检 · 设置；含中文的脚本与 C# 源码补上 UTF-8 BOM（冻结的 Capture 源码保持逐字节不变），在英文 Windows 上也能正确构建和运行；发布构建不再要求仓库里存在未跟踪的 VB-CABLE 驱动包，缺失时由安装器在线下载并校验。
...
1. **配对遥控器**：在 Windows 蓝牙里连接 `MI RC` / `RC003`，回到 Vibe Link 点「重新检测」，按一次方向键确认收到的是遥控器事件。
2. **选语音工具**：在「语音」页选 微信输入法 / 网易八哥说 / 其他语音工具，并把工具自己设置里的启动快捷键改成与页面**完全一致**。
3. **添加应用并设为当前**：在「工作流」页添加常用应用、学习它的输入框、测试通过后「设为当前」。
4. **按住说话**：点击输入框后按住遥控器录音键说话，松开等待文字，检查后按确认键发送。文字不对时，先看「语音」页的最近一次收音电平。
...
当前构建没有商业 Authenticode 签名，Windows 会显示「未知发布者」或 SmartScreen 提示。处理步骤：在提示窗口点「更多信息」，确认来源是你下载的 GitHub Release 地址后点「仍要运行」。请先核对下载地址和随包 `SHA256SUMS.txt` 中的 SHA-256，不要关闭 Windows 安全功能或 SmartScreen。

## 首次设置五项任务

首次启动会引导完成五项任务：确认设备与用法 → 连接并测试遥控器 → 准备本地音频通道 → 选择工具并完成听写 → 开机即用。每一步都可以稍后处理，进度会保留；点击按钮本身不会自动变成“已完成”，只有检测或用户明确确认后才会改变状态。

### 任务 1：了解设备、流程和隐私

![首次设置：了解设备和流程](images/00-setup-01-device.png)
...
这里还会说明：RC003 单段约 60 秒、录音键不可改成点击切换、Vibe Link 不做语音识别，也不保存普通录音和转写文字。

### 任务 2：连接并验证遥控器

![首次设置：连接遥控器](images/00-setup-02-remote.png)

1. 打开 Windows 蓝牙设置。
2. 配对 `MI RC` / `RC003`。
3. 返回 Vibe Link，点击“重新检测”。
...
> 界面与任务 2 同构（左侧五步进度 + 右侧操作区）。向导的每一步都要**先在任务上完成一次真实操作**才会放行下一步，所以这里只给出任务 1、2 的真实截图，而不是贴与上一步相同的画面。

完整 RC003 麦克风链路需要 VB-CABLE：

```text
RC003 麦克风 -> Bluetooth ATVV -> Vibe Link -> CABLE Input
                                                       |
...
官方驱动安装需要用户确认和可能的管理员权限。安装器退出不等于音频链路已经可用，必须返回 Vibe Link 重新检测并完成真实测试。

### 任务 4：选择工具并完成一次真实听写

> 这一步要求**真实出字**才会放行；截图与任务 2 同构，故不重复贴图。
...
> 最后一步只做后台与开机行为的勾选，界面与任务 2 同构，故不重复贴图。
...
![语音页：音频端点和最近一次收音电平](images/02-dictation.png)

### 正确方向

| 位置 | 应选择 |
| --- | --- |
| Vibe Link 播放端 | `CABLE Input` |
| 语音工具的麦克风输入 | `CABLE Output` |
...
当前候选版提供三个可选语音工具，顺序与「语音」页一致：

1. **微信输入法**（默认，`Ctrl + Win`，单击切换）；
2. **网易八哥说**（右 Alt，单击切换）；
3. **其他语音工具**（自定义，默认右 Shift，单击切换）。

![语音页：三个可选转写工具](images/06-transcription-tools.png)

Typeless、Windows 语音输入、讯飞语音输入法、搜狗输入法和豆包输入法都已从 V2.0 集成中移除，不会出现在「语音」页或首次设置中。旧配置里保存的这些值会在加载时按未知 provider 规则回退到微信输入法的稳定参数，并显示一次迁移提示。

无论选哪一个，应用里保存的快捷键和触发方式都必须与工具自己配置的完全一致，否则按下录音键不会启动听写。

免驱动模式（未检测到 VB-CABLE、没有采集进程）下，主机能自行唤起网易八哥说和自定义工具；微信输入法需要它自己的语音面板，因此仍需要 VB-CABLE。


### 微信输入法

微信输入法仍是默认工具，稳定档案是 `Ctrl + Win` 单击切换。它是第三方工具：部分版本会先把识别结果放入剪贴板，而不是直接向当前控件注入文字。Vibe Link 不读取剪贴板文字，也不能保证它在所有 ChatGPT、浏览器或提权窗口中直写。
...
1. 在「语音」页把「转写工具」选成 **网易八哥说**；默认快捷键会填 **右 Alt**（`rightalt`）。
2. **在网易八哥说自己的设置里确认它的启动/结束快捷键就是右 Alt**；如果它在客户端里用的是别的组合，请把语音页的快捷键改成**完全一致**的那个（应用与工具两边不一致时，按键按下不会启动听写）。
3. 保持「单击切换」触发方式（默认）。先在键盘上按一次右 Alt 确认网易八哥说能启动与结束，再用遥控器录音键测试。
4. 先不要依赖自动粘贴：网易八哥说是否把文字直接写进输入框**尚未在本机实测**，所以 Vibe Link **不会**为它派发 `Ctrl+V`。如果实测发现它其实把结果放进剪贴板，需要在语音页改用「其他语音工具」并手动粘贴，或等待我们对它的投递方式做实测后适配。
5. Vibe Link 只负责派发触发动作与维持录音，**不读取**网易八哥说的转写内容。

### 其他语音工具

把「转写工具」选成 **其他语音工具** 后，默认快捷键是**右 Shift**（`rightshift`），触发方式是「单击切换」。请在你自己的工具里把启动/结束快捷键设成同一个组合，先在键盘上验证一次，再用遥控器录音键测试。
...
3. 点击“学习目标输入框”。Vibe Link 会暂时隐藏主窗口并显示简短提示。
...
如果你已经手动点击了一个有效输入框，手动焦点优先；Smart Focus 不应在录音过程中抢走焦点。目标失效、应用关闭或当前进程不匹配时，Vibe Link 会停止后续聚焦动作并显示修复入口，不会静默向未知位置发送内容。
...
- TV：默认任务视图，可配置；
- 功能键：短按复制、长按粘贴，可分别修改；
- 电源键：普通可映射键，短按 / 长按 / 双击各挂一个动作（本机当前为 DeepSeek 用量页 / Bilibili / 任务切换），它不再作为录音键候选；未指派时交给 Windows。返回键和独立音量键在当前 RC003 / Windows 组合中没有稳定事件，不提供映射。
...
![录制键盘快捷键：只在录制窗口打开期间拦截](images/08-shortcut-recorder.png)
...
| 文字出现后被替换 | 检查微信输入法是否重复唤起或开启 AI 整理 | 不要把第三方工具状态误报成 Vibe Link 已发送 |
...
- 遥控器在线时录音键（F5）被拦截；遥控器不在线时普通键盘 F5 原样直通到前台应用，这是已披露的限制，不等同于带签名的设备级隔离。
...
如果问题仍存在，请提供：Vibe Link 版本、Windows 版本、语音工具、是否安装 VB-CABLE、目标应用、当前自检错误码以及“按键后发生了什么”的最短复现步骤。不要上传私人录音、转写文字或包含个人信息的截图。

<p align="center">
  <img src="images/vibe-flow-community.png" alt="扫码加入 Vibe Link 用户社区" width="760">
</p>
