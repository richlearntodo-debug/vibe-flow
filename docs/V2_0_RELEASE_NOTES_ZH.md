...
- 可选语音工具为微信输入法（默认，`Ctrl + Win`，单击切换）、网易八哥说（右 Alt，单击切换）、讯飞语音输入法（`F6`，按住触发）、搜狗输入法（右 Ctrl，按住触发）、豆包输入法（`Alt + 空格`，按住触发；支持但未验证）和其他语音工具。
- 讯飞语音输入法为本候选版新增支持。请在讯飞输入法「设置 → 语音」把语音快捷键设为 `F6`（讯飞里唯一可设置的语音快捷键），并选择「长按说话」；Vibe Flow 里保存的快捷键和触发方式必须与它完全一致。
- 搜狗输入法为本候选版新增支持（本机安装于 `D:\SogouInput\16.6.0.4073`）。请在搜狗自己的语音/按键设置里把语音输入快捷键设为右 Ctrl 并保持「按住说话」，Vibe Flow 里保存的快捷键和触发方式必须与它完全一致。
- 豆包输入法在本候选版重新提供（它此前曾被下线，现按用户要求恢复），存储 `Alt + 空格` 加按住触发。它是**支持但未验证**：其语音面板可能忽略模拟按键，其麦克风列表也可能不提供虚拟声卡 `CABLE Output`；请先按未验证路径对待，不要当成已经可用的工具。
- 微信输入法部分版本会把识别结果放到剪贴板，属于第三方工具行为。候选版可能在目标已验证且收到真实完成回执后派发一次 `Ctrl+V` 兼容动作，但不保证所有目标都能成功粘贴；“粘贴动作已派发”不等于 AI 已收到。
- Typeless 和 Windows 语音输入不再作为 V2.0 的语音工具选项，保存的旧值会在加载时迁移为微信输入法并按名称提示用户。
