...
>
> **录音键隔离**：未安装签名设备过滤器前，录音键使用“遥控器在线范围内”的钩子隔离——遥控器已连接时 F5 被拦截，遥控器不在线时普通键盘 F5 原样直通，且用户态无法做逐事件设备归属隔离。它与签名过滤器不等同，详见[已知限制](docs/V2_0_KNOWN_LIMITATIONS_ZH.md)；安装后在「首页」「快捷键」「自检」与首启向导第一步都会看到当前状态。
