...
- 提供 11 步首次设置、10 项自检、可即时切换的白天/夜间主题和隐私安全日志。
...
1. 打开左侧“快捷键”。
2. 选择上、下、左或右中的一个方向。
3. 点击“设为区域截图”。也可以在“执行动作”中选择“系统 · 区域截图”。
4. 点击“立即测试”；Windows 应显示截图选区。
5. 配置会立即保存并应用；需要时仍可点击“保存并应用”重启按键桥接。

配置后，该方向键在普通页面中不再执行原方向，而是发送 `Win + Shift + S`。需要恢复时点击“恢复方向导航”。TV 任务视图打开期间，四个方向仍优先用于选择程序，不会误触截图。

## 切换白天与夜间模式

![白天与夜间模式按钮](images/05-settings.png)

1. 打开左侧“设置”。
2. 点击“白天模式”或“夜间模式”。
3. 页面会立即完成切换并保存选择，不需要关闭或重启言灵。

主题只改变界面颜色，不会修改录音参数、蓝牙连接、语音工具或快捷键。
...
自检覆盖核心组件、蓝牙、遥控器、按键、真实音频、VB-CABLE、稳定参数、语音工具、自启动和完整会话。当前规则按稳定录音内核 `v1.0.3 / v11` 判断组件状态，不再要求已经移除的长录音状态机。优先处理第一张红色或橙色卡片，点击卡片可进入相应 Windows 设置或应用页面。
