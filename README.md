...
5. 在“语音”页选择语音工具：微信输入法（默认，`Ctrl + Win`，单击切换）、网易八哥说（右 Alt，单击切换）、讯飞语音输入法（`F6`，按住触发）、搜狗输入法（右 Ctrl，按住触发）、豆包输入法（`Alt + 空格`，按住触发；支持但未验证）或其他语音工具。选讯飞语音输入法时，请在讯飞输入法「设置 → 语音」把语音快捷键设为 `F6` 并选择「长按说话」；选搜狗输入法时，请在搜狗自己的设置里把语音输入快捷键设为右 Ctrl 并保持「按住说话」；两者都必须与言灵保存的值相等。豆包输入法是支持但未验证的路径：它的语音面板可能忽略模拟按键，麦克风列表也可能不提供 `CABLE Output`。微信输入法的语音面板行为由第三方工具决定，可能把文字放到剪贴板。
...
- 未检测到 VB-CABLE（免驱动模式）时，主机能自行唤起网易八哥说、讯飞语音输入法、搜狗输入法和自定义工具；微信输入法需要它自己的语音面板，因此仍需要 VB-CABLE。
