...
需要排查“文字进剪贴板”时，改用讯飞语音输入法做对照：在讯飞输入法「设置 → 语音」把语音快捷键设为 `F6`（讯飞里唯一可设置的语音快捷键），并选择「长按说话」，与言灵保存的快捷键和触发方式保持一致。微信输入法部分版本会把识别结果放入剪贴板，这是第三方行为；Vibe Flow 不读取转写文字，也不保证所有微信输入法目标都能直写。
