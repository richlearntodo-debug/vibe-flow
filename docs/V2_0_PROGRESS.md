...
### 3/3 再追记：Live HUD 已实测（累计 **13** 个界面），并纠正我自己一个错误假设

**我先前的假设是错的**：我把 Live HUD / Context Deck / Capture & Ask 归为"需要真实录音或托盘触发，本机无条件"。本轮证明**前一个不需要**——在 smoke 里点「启动语音桥接」就会开始一次会话，随后出现一个新窗口：

```
[] class=WindowsForms10.Window.8...  400x160
dialog=handle:…  size=400x160   dialog: overlaps=0   dialog: clipped=0
    [STATIC] 正在检查
    [STATIC] （图标字形）
    [STATIC] 正在连接遥控器麦克风，请稍候
    [STATIC] 真实音频峰值 · 等待一次真实听写
    [STATIC] 目标 · Capture Smoke · 已验证 · 应用未运行 ｜ Profile · 通用导航
```

**实测**：**400×160**（设计 200×80）→ 恰好 ×2，**0 重叠 / 0 裁切** ✔ 并且内部实时文案都在正常渲染。

它**没有标题**（`FormBorderStyle.None` 的无边框窗口），所以按标题那种测法根本找不到它——为此给几何检查加了 **`-WindowHandle`** 模式（按句柄测量），并进门禁。这也解释了它为什么会被我误判成"够不到"。

#### 另实测：两个删除确认框（都是**系统对话框**）

| 确认框 | class | 尺寸 | 结果 |
| --- | --- | --- | --- |
| 删除快捷键 Profile | `#32770` | 664×317 | 0 重叠 / 0 裁切 |
| 删除常用应用 | `#32770` | 969×317 | 0 重叠 / 0 裁切 |

`#32770` 是 Windows 标准对话框类（MessageBox/TaskDialog）→ **布局是操作系统的，不是我们的**，所以这里没有"我们的设计"需要验证；记录它们是**为了让这一项不因错误的原因空着**。测量时**一律取消、不确认**，两个确认框守护的状态未被改动；播种的常用应用状态也在 finally 里清理并验证已删除 ✔。

#### item 3 界面累计：**13 个已实测**（全部恰好 2× 设计尺寸、0 重叠、0 裁切）

绑定 Smart Profile 应用 · 新建 Profile · 重命名 Profile · 添加应用 · Browser Remote Lite · 调整高级音频参数 · 配置 上键短按 · 录制键盘快捷键 · 用语片段 · 编辑常用应用 · 删除 Profile 确认框 · 删除常用应用确认框 · **Live HUD**

#### 只剩 2 个：都是**托盘菜单**入口（附代码证据）

| 界面 | 入口 | 证据 |
| --- | --- | --- |
| Context Deck（遥控器状态） | 托盘菜单「打开遥控器状态」 | `VibeMic.cs:10584  menu.Items.Add("打开遥控器状态", …, delegate { ShowContextDeck(); });` |
| Capture & Ask（截图提问） | 托盘菜单「截图提问」 | `VibeMic.cs:10585  menu.Items.Add("截图提问", …, delegate { ShowCaptureAsk(); });` |

即：**不在应用自身的界面路径上**，要触发就得去点系统托盘图标（涉及用户的任务栏）。**我没有去动用户的托盘**——这属于"可以自动化但会碰用户 shell"的一类，需要你同意才做。
**可选的下一步**：我用经典的托盘自动化（找到托盘工具栏 `ToolbarWindow32` → 用 `TB_GETITEMRECT` 定位图标 → 向其投递右键）来打开该菜单并点这两项；或者你手动右键托盘图标点这两项，我立刻用 `-WindowHandle`/`-WindowTitle` 把它们量掉。

### 3/3 再追记：托盘专属界面改为**应用内断言**（累计覆盖 14 个界面）

Context Deck 与 Capture & Ask 只有托盘菜单入口，**不在应用自身界面路径上**。我没有去动你的托盘，而是把**同一套几何规则搬进应用的自测**：

- 新增 `AssertSurfaceGeometry(Form, Size designSize, string name)`：断言**窗体尺寸 = 设计尺寸 × 当前显示缩放**（±2px）**且内部无兄弟控件重叠**（阈值 2px，与外部检查脚本同一规则）；
- 在既有 UI 自测里对 **Context Deck** 调用它 ✔ **随发布链每次构建运行**（比一次性人工测量更强）。

#### 写断言前先测量（这次又纠正了我两个假设）

1. 我先按"820×696 × 2 = 1640×1392"去要求，但实测既有自测里的 deck 是 **1342×704** ✗；
2. 追下去发现：**1342 = 1366−24、704 = 728−24** —— 那是自测**故意**把 deck 塞进模拟的 1366×728 工作区（`VibeMic.cs:2636`）✔ 所以那里的尺寸与缩放无关，**我的期望用错了场景**；
3. 于是单独构造一个**新 deck 并显示**（拟合真实工作区 2560×1440）→ 实测 **1640×1392 = 820×696 × 2** ✔ **缩放正确**，据此才写死断言。

#### 负对照（证明断言会咬）

把设计尺寸故意写错成 800×696 → 自测**失败并报**：

```
Vibe Flow host self-test failed: Context Deck is 1640x1392 where its design size at this display scaling is 1600x1392
```
...
- **13 个外部实测**（几何检查脚本）：10 个对话框 + 2 个删除确认框（系统对话框 `#32770`）+ Live HUD（400×160，无标题，为此新增 `-WindowHandle` 模式）
- **1 个应用内断言**：Context Deck（托盘专属）

#### 只剩 1 个：Capture & Ask

原因是**技术性的、已查明**：它的服务需要**实例状态**（状态根目录、焦点目标服务、录音优先级门闩），而承载该自测的方法是 **static** 的，无法构造它 ✗。两条可选路径（下次即可执行）：

1. 自动化托盘图标（定位 `ToolbarWindow32` → `TB_GETITEMRECT` → 投递右键）——会**动到你的任务栏**，需要你同意；
...
## 2026-09-11 修复：Capture & Ask **被缩放两次**（它会铺满整个屏幕）

### 缺陷与修复

上一轮记录为 `captureAsk=MISMATCH shown=2536x1416 expected=1560x1400` —— 2536×1416 恰是**工作区减边距**，即该窗体**铺满屏幕**而不是呈现设计尺寸 ✗

**根因（逐步测量得出，不是推断）**：`CaptureAskForm` 设了 `AutoScaleDimensions = new SizeF(96f, 96f)` + `AutoScaleMode.Dpi`，**Windows Forms 自己会缩放它**（实测：构造函数结束时已是 1534×1329，而设计是 780×700）；`UiDisplayScale.Apply` 在 Load 时**又缩一次** → 尺寸超出工作区 → 它自己的 `ApplyWorkingAreaLayout` 把结果**夹到工作区** ✗

**修法**：把该窗体从 `UiDisplayScale` **摘除**（它已由 WinForms 负责缩放）✔

**修复后实测**：`shown=1534x1329 client=1508x1258` ✔ —— 客户端区 **1508×1258 恰好 = 设计客户端(754×629) × 2** ✔✔

### 一个我必须写清的细节：窗口尺寸 ≠ 客户端区尺寸

我原先把"设计**窗口**尺寸 × 缩放"当作期望值 ✗，于是留下 −26×−71 的"差值"疑云。实测一量：`window − client = 26×71` —— **正是那个差值** ✔ 结论：**WinForms 缩放的是客户端区，非客户区（边框/标题栏）不参与** ✔。所以正确的判据是**客户端区**，不是窗口尺寸。

### Browser Remote Lite：**改了又撤回**（未经验证就不改）

它和 Capture & Ask 是同一代码模式（`AutoScaleDimensions=(96,96)` + `Apply`），所以我一并摘除了。但实测变成 **1654×1369** ✗ ——**既不是**设计×2（2536×1416），**也不是**工作区夹取值 ✗，即**无法判定是改善还是劣化** ✗ → **已还原**（回到 2536×1416、0 重叠、0 裁切 ✔），并在代码与文档里写明：它需要和别的窗体一样先做"构造时 vs 显示后"的测量，再动 ✗。

**这条值得作为纪律记下**：上一轮我因为"只看最终尺寸无法区分两种成因"而发现了 bug；这一轮我差点因为"改完数字变了就算修好"而提交一个未验证的改动 ✗。

### 顺带纠正我自己的一个错误数据

我给 LiveHud 的日志里写了**设计尺寸 200×80**（凭上一轮外部实测反推的 ✗），于是它显示 800×320 时被算成"ratio=4.00 双重缩放" ✗。查代码：它的设计是 **400×160**（`LiveHudForm.cs:52`）→ 800×320 = **恰好 ×2 ✔ 本来是对的** ✔。日志已改用真实设计尺寸。

### 本轮结论（三窗体测量表）

| 窗体 | 构造时 | 显示后 | 结果 |
| --- | --- | --- | --- |
| ContextDeck | 820×696 | 1640×1392 | ratio 2.00 ✔（并已有应用内断言） |
| LiveHud | 400×160 | 800×320 | ratio 2.00 ✔ |
| AppPicker | 580×660 | 1160×1320 | ratio 2.00 ✔ |
| **CaptureAsk** | 1534×1329（WinForms 已缩放） | 1534×1329 | 客户端区 = 设计 × 2 ✔ **（本轮修复）** |
...
## 2026-09-11 收口我上一轮标出的两个验证空档

### ① 导出诊断的内容：现在可测且被测

**问题**：导出走系统「另存为」对话框 ✗，无法在测试里驱动，所以"报告里到底有没有崩溃信息"只靠门禁钉字符串 ✗。

**做法**：把「生成报告」与「问用户存到哪」拆开 —— 新增 `BuildDiagnosticsReport()`，`ExportDiagnostics()` 只负责对话框 + 写文件 ✔（顺带让导出逻辑可测 ✔）。

**在每次 smoke 运行里断言**（因此 12 例界面矩阵各跑一遍）：报告非空、必须含 `Technical diagnostics / Generated / Windows / App / Capture running / UI rendering / Display / Crashes recorded / Mappings`，并且崩溃计数可解析 ✔。

**并且把"包含最新崩溃"也断言上**（当存在崩溃报告时）✔ —— 这条**真的被跑到了**：当时数据目录里正好有 2 份崩溃报告（**是我这次开发中自己断言失败留下的** ✔），报告构建结果 `crashes=2 includesNewest=True` ✔✔ 之后我清掉了那两份开发残留（不是真实用户崩溃 ✔），现在同一行为 `crashes=0 includesNewest=False` ✔ 两种情形都实测过 ✔。

> 附带一个有意思的证据链：那两份崩溃报告的内容正是 `Capture & Ask is 2536x1416 where its design size ... is 1560x1400` 和 `ContextDeck is 1614x1321 where ... is 1588x1250` —— **是我自己的断言在真实运行中触发、并被崩溃记录功能如实记下** ✔ 这同时证明了崩溃记录与几何断言两条链路都在工作。

### ② Tab 顺序：从"只记录"变成"有断言"

原来的走查只写日志 ✗（一次报 0 也可能只是没测到 ✗）。现在每次 smoke 运行会断言三条：

1. 每个页面**至少能到达一个控件**（否则该页键盘不可用 ✗）；
2. **绝不访问"刻意不可聚焦"的控件**（`visitedNonTabStop == 0` —— 这是走查的负对照 ✔，即"这个数字确实在量 Tab 顺序"）✔
3. **可达数不得明显低于该页声明的 TabStop 数**（`visited ≥ tabbable − 2`）—— 这是**我修过的那个 bug 的回归测试**：当年按"控件名字"判重，遇到同名按钮就提前结束，在自检页报出 **1** 个可达控件（实际 34 个）✗ → 这条下界会当场抓住它 ✔

六页当前实测（断言通过）：`12/11`、`7/7`、`66/66`、`13/15`、`34/34`、`20/20`，且每页顺序的**末尾都是六个导航键**（首页→工作流→快捷键→语音→自检→设置）✔

### 仍未做（下一步）

- **深色主题下只验过六个页面** ✗：**对话框与首次设置向导的深色渲染从未逐一看过** ✗（我的对话框驱动脚本与截图脚本都按浅色启动 ✗）。这是深色路径上剩下的真实风险面 —— 下一轮把 `--ui-theme dark` 接进这两个驱动，把深色下的对话框与向导逐个量一遍 ✔
- 矩阵 `1366x768`/`880x500` 两档在 200% 下被缩放下限夹到 1760×1000（行为正确、标签名不副实）——文档已说明 ✔

## 2026-09-11 深色路径与低缩放补齐（含一个重要的环境变化）

### 环境变化（影响对既有结论的解读）

本轮中途测得 **显示缩放已回到 100%**（`UI RENDER … dpi=96 scale=1.00 window=1280x840 sidebar=232 content=1015`）——**不是我改的** ✗。含义：
- 此前所有"200% 下"的验证**在当时是真实有效的**（日志里有 `dpi=192 scale=2.00` ✔），但**现在**机器状态是 100%；
- 这反而把我一直标注为"需要你切回 100% 才能测"的那一项**变成了可测** ✔ → 见下。

### ① 小屏窗口尺寸：**终于在正确缩放下真测了**（此前是 200% 下的夹取）

12 例矩阵在 100% 下逐个断言窗口尺寸**恰等于被强制值**：

```
light/dark/system × {默认, 1366x768, 1920x1080, 880x500}
  default      window=1280x840
  1366x768     window=1366x768    ← 不再被夹
  1920x1080    window=1920x1080
  880x500      window=880x500     ← 真正的最小窗口
全部 6 页 / 0 重叠 / 0 裁切
```

（此前 200% 下这两档被缩放下限夹到 1760×1000 ✗ —— 行为正确但**没测到真实小屏** ✔ 现在补上了 ✔）

### ② 深色主题：向导与对话框**首次实际查看**

- **首次设置向导**：以 `--ui-theme dark` 启动后截图，背景像素 **(25,26,31)** = 深色 ✔ 渲染正确、文字可读 ✔（此前只在浅色下看过 ✗）
- **对话框**：深色下逐个测量（重命名 Profile / Browser Remote Lite / 添加应用 / 调整高级音频参数）**全部 0 重叠 / 0 裁切** ✔ 并查看截图：深色、可读 ✔
  - **观察（不下结论）**：Browser Remote Lite 的表头与表格行在深色下**仍是浅色**（白底黑字）✗——与周围深色页面不一致，但**可读** ✔ 是否有意为之未验证 ✗

### ③ 一个**假缺陷**：运行中切换主题的截图是过期的

截图脚本 `-Theme Dark` 是**在运行中点击设置页的主题单选按钮** ✔。那次的截图显示：主窗口仍是**浅色**、同时屏幕上飘着「已切换到夜间模式」的提示 ✗ → 看起来像"提示说切了、其实没切" ✗✗。

**用测量判定**（点击后 2s/5s/9s 各抓一次，读背景像素）：

```
before : RGB=(248,249,252) luminance=250 -> light
after 2s / 5s / 9s : RGB=(35,37,44) luminance=39 -> dark
```

→ **运行中切换是生效的** ✔（2 秒内变暗并稳定 ✔）。那次截图是 **PrintWindow 抓到了刚重建完的界面上的陈旧表面** ✗ —— **不是产品缺陷** ✗。
**结论**：深色截图应以**启动时应用主题**的方式获取 ✔（本轮向导与对话框都是这么做的 ✔）；运行中切换则用**像素测量**判定 ✔。

### 本轮仍未做

- 深色下**其余对话框**（动作选择器 / 录制快捷键 / 用语片段 / 编辑常用应用 / 两个删除确认框）未逐个查看（尺寸已测的四个已看 ✔）；
- CI 未跑（分支未推送 ✗）；
- 真机/授权类（RC003 隔离、签名、干净机器安装、无 CABLE 首启）✗。

## 2026-09-11 首次设置向导补上「录音键隔离」披露（发布前最重要的一条提示）

**为什么做**：设备级按键隔离是本项目最大的发布顾虑——当前为「Raw Input 安全直通」，**前台应用可能同时收到录音键** ✔（首页琥珀色提示 / 快捷键页徽章 / 自检页 / 导出诊断里都有 ✔），但**首次设置向导里没有** ✗。新用户第一眼看到的应该是这条，而不是在后续某个页面的角落里发现 ✗。

**做法**：在向导第一步（确认设备与用法）的「适用」行下方加一条琥珀色提示，**仅在当前确实不健康时显示**：

```
!  录音键当前为安全直通：前台应用可能同时收到录音键；设备级精确隔离仍在开发中。
```

- 条件用 `ReadKeyboardBridgeHealth().FilterHealthy` ✔（我先写了 `exactDeviceIsolation` ✗ —— 那是设置页里的**局部变量** ✗，编译报错后**回代码查证**才找到正确来源 ✔ 没有猜 ✔）
- 位置 y=494、尺寸 690×28，第一步内容区高 572 ✔ 有余量 ✔
- **验证**：重新截图并查看 ✔ 提示行**完整显示、未裁切**，第一步其余内容**未位移、未重叠** ✔；出厂截图 `docs/images/00-setup-01-device.png` 已同步替换 ✔
- **门禁**：钉住 `onboardingKeyIsolationNotice` 这个控件名、提示原文、以及"仅在 FilterHealthy 为假时显示"的条件 ✔ —— 防止这条披露被无声删掉 ✔
- 发布清单同步更新（原写"向导里没有提示" ✗ → 现已加入并截图确认 ✔）

## 2026-09-11 深色模式：修掉"表格行在深色下仍是白底"

**观察→定位→修复→测量**（上一轮我只记录"观察"没有下结论 ✔）：

- **观察**：Browser Remote Lite 在深色下**表格行是白底黑字** ✗（与周围深色页面不一致 ✗）
- **定位**：它是 `ListView`（`View.Details` ✔），而 `ApplyTheme` **确实**给它设了 `BackColor = (35,37,44)` ✔ → 说明不是配色表漏了 ✗，而是 **WinForms 的已知行为：启用视觉样式时，带主题的 ListView 会忽略 `BackColor`** ✗
- **修复**：对**这一个控件**关闭主题（`SetWindowTheme(handle, "", "")` ✔ 文档给出的做法 ✔）
- **测量**：修复后 5×5 网格采样，表格行区域 = **(35,37,44)** ✔ = 深色表面色 ✔；几何仍为 840×720、0 重叠 / 0 裁切 ✔

**仍未解决的一小块（已如实记录，未假装修好）**：ListView 的**列头**是独立的 `SysHeader32` 窗口 ✗，即使对它也关闭主题，实测**仍是浅色**（采样 240,240,240 ✗）→ **该调用已删除** ✗（不留无效代码 ✔），并写明"要上色需要自绘" ✔。

> 这里也踩到一次"截图与像素不一致"：我先把表格区域看成整片白 ✗，单点采样却报深色 ✗——**换成网格采样**才看清：**行是深色 ✔、列头仍是浅色 ✗**。单点会落在网格线/高亮上 ✗，这是个值得记住的采样教训。

**新增门禁**：钉住 `SetWindowTheme` 的声明与调用、"列头需要自绘"的说明 ✔ —— 防止这条修复被无声移除（那会让行重新变白 ✔）。

## 2026-09-11 发布面向用户的文档补上「录音键隔离现状」（发布清单的首要条件）

**发现的缺口**：应用内有提示（首页 / 快捷键 / 自检 / 首启向导第一步 ✔），**但用户下载前会读的文档里全都没有** ✗：

- `README.md`：5 处提到"录音键"，**全部是在讲怎么用** ✗，没有一处说明隔离现状 ✗
- `QUICK_START_ZH.md`：3 处，同样只讲用法 ✗
- **发布说明模板 `docs/GITHUB_RELEASE_BODY_ZH.md`**：只在"仍未验证"清单里出现"普通键盘 F5 隔离" ✗ —— **不是给用户看的风险说明** ✗

这正是发布清单写的首要条件「下载页/发布说明里显式写出」✗ —— 之前**没有满足** ✔（本轮补上）。

**做法**（三处都加，措辞以 `V2_0_KNOWN_LIMITATIONS_ZH.md` 的**精确行为**为准，而不是笼统的"可能收到"）：

- 未安装签名设备过滤器前，录音键使用**"遥控器在线范围内"的钩子隔离**，不是逐事件设备归属隔离；
- 遥控器**已连接**时 F5 被拦截，**不会**触发浏览器刷新或页面动作；
- 遥控器**不在线**时，普通键盘 F5 **原样直通**；
- 用户态无法逐事件归属，因此**不能与签名过滤器等同**；逐键/普通键盘 F5 隔离矩阵**尚未完成**。

**验证**：重新跑发布链后，**生成的** `release/RELEASE_BODY_v2.0.0.md` 里确实出现了这一节 ✔（不是只改了模板 ✔）；`validate` 通过 ✔。
**门禁**：钉住三份文档各自的这一段（含"原样直通""不能与签名过滤器等同"等关键句 ✔），防止被删 ✗。

## 2026-09-11 应用工作流：从「自检」移出，只留在「工作流」页，并简化

### 你提出的问题（已复现确认）

「自检」页在「检查结果」之上还有一整块「应用工作流」✗：**14 个应用、每个 4 行** —— 正确状态 / 当前状态 / **原因 [VF-WORKFLOW-CURSOR]** / 下一步 ✗ → 13 个应用就是 **52 行文字**堆在一张本该只回答"组件是否正常"的页面上 ✗。这正是你说的"一堆的报错" ✔。

### 改动

**① 自检页整块移除** ✔ 现在页面 = 概览（真实的 2 项错误）+ 图例 + **检查结果** + 高级诊断 ✔，工作流不再出现在这里 ✔。

**② 工作流页新增紧凑的「应用工作流」卡（按你选的方案）** ✔
- 顶部一句**中性汇总**：「14 个应用：0 个已就绪 · 13 个还没学习」✔
- **只列还需要处理的应用，一行一个**（已经就绪的只计入汇总、不占位置 ✔）：状态点 + 应用名 + 一行缺口 + 「键位/工作流/语音工具」的紧凑现状 + **一个**动作按钮 ✔（动作与文案都取自卡片模型，点击走**原来那个**动作分发器 ✔，所以行为和以前一致 ✔）
- 全部就绪时只显示一句"所有应用都已就绪，无需处理" ✔

**③ 状态措辞改成"进度"而不是"故障"** ✔（也符合产品自己文档里的状态词）：
`缺少工作流` → **`还没学习`**；`工作流未验证` → **`已学习未验证`**；`缺少键位 Profile` → **`还没选择键位`**；`尚未真实验证` → **`还没真实验证`** ✔

### 测量验证

| 项 | 结果 |
| --- | --- |
| 自检页可达控件（Tab 顺序断言） | **34 → 20** ✔（正好少掉 13 个应用按钮 + 1） |
| 工作流页可达控件 | **7 → 21** ✔（正好多出 14） |
| 两页几何 | 1280×840，**0 重叠 / 0 裁切** ✔ |
| 键盘走查三条断言 | 全部通过（含"绝不访问刻意不可聚焦控件"的负对照 ✔） |
| self-test / validate / 矩阵 12 例 | 全部通过 ✔ |

### 顺带清掉的死代码

`BuildWorkflowCardItems`（把工作流卡片转成自检行的适配器）在移出后**已无人调用** → 删除 ✔（不留死代码 ✔）。

### 一个仪器教训（值得记）

改完标签后截图**逐字节没变** ✗，一度像是"改动没生效" ✗。改用**读控件文字**（而不是看像素）后确认：应用实际渲染的就是新措辞 ✔ —— **`PrintWindow` 在页面刚重建后可能返回陈旧画面** ✗（和之前"运行中切主题"那次同一现象 ✔）。
**结论：像素截图用于"看布局"可以，用于"判断文字/状态是否更新"不可靠；控件文字与几何矩形才是权威判据** ✔（几何数字来自窗口矩形，因此不受此影响 ✔）。
...
## 2026-09-11 「添加应用」对话框重做：每行都有图标、界面重做、行内即时筛选

你提的三点：**很多应用没有 logo** ✗、**界面不好看** ✗、**要能直接筛选（不分步骤）** ✗ —— 处理如下。

### ① 图标：不是"有些没取到"，而是**取不到时会留空**

原因查明：行是自绘的 ✔，但只有 `item.Icon != null` 时才画 ✗。而目录扫描在本机实测确实**经常给不出图标**（可执行文件本身没有图标资源 / 通过 AppUserModelID 解析的打包应用 / shell 能画出但提取不到 ✗）→ 于是列表里**一行有图、一行空白** ✗，看起来像坏了 ✔。

**现在是一条兜底链**（按序尝试 ✔）：目录已解析的图标 → **该进程可执行文件的图标** → **启动目标的图标** → **按应用名生成的字母牌** ✔（名字哈希决定底色，同一个应用每次都一样 ✔）。
**结果：没有任何一行是空槽** ✔ —— 截图里 ChatGPT 那行就是生成的字母牌 ✔（它的图标提取不到 ✔）。

### ② 界面重做

| 之前 | 现在 |
| --- | --- |
| 纯白背景、系统默认按钮、行首编号「01 / 02」 | 与主程序同一套配色 ✔（紫 104,82,244 主色 ✔）；标题+副标题层级 ✔ |
| 筛选框是一条朴素输入框 | 圆角搜索框 ✔ + 放大镜图标 ✔ + **占位提示**「搜索应用名或进程名，例如 cursor / chrome」✔ |
| 行：图标 + 名称 + 状态 | 行：图标牌 + 名称 + 状态点 ✔（正在运行为绿色 ●、已安装为灰字 ✔）+ **选中行紫色左条** ✔ + **悬停高亮** ✔ |
| 底部无计数 | 底部实时计数：「共 95 个已安装的应用」/「显示 1 / 95 个应用」✔ |
| **完全没有深色主题** ✗（硬编码白色 ✗） | **跟随白天/夜间** ✔（实测深色截图：深底、浅字、紫色主按钮 ✔） |
| 行内偏移是固定像素 ✗ | 行内所有尺寸**按行高比例算** ✔（高 DPI 下行高被缩放而常量不会 ✗ → 之前 200% 下图标与两行文字会挤在加高行的顶部 ✗） |

### ③ 筛选：**本来就没有步骤**，这次把体验做实

筛选框**打开即聚焦** ✔、**逐字即时过滤** ✔（名称或进程名都匹配 ✔）、底部显示"显示 N / 共 M" ✔、**没有匹配时**显示居中提示「没有匹配的应用。换个词试试，或清掉筛选看全部。」✔、清空后占位提示回来 ✔、**回车直接确定** ✔（`AcceptButton` ✔）。

**用 WM_SETTEXT 从外部真实驱动验证过**（不是看代码相信 ✔），并**只统计可见控件**（普通枚举会把隐藏控件也列出来，会误导 ✗）：

```
筛选「zzzz」→ 可见：[没有匹配的应用。换个词试试，或清掉筛选看全部。] + [共 95 个已安装 · 当前筛选没有结果]
筛选「chrome」→ 可见：[显示 1 / 95 个应用]（空状态提示已隐藏 ✔）
清空筛选     → 可见：[搜索应用名或进程名，例如 cursor / chrome] + [共 95 个已安装的应用]
```

### 几何与门禁

- 对话框 **600×700**，亮色与深色下均 **0 重叠 / 0 裁切** ✔（`scripts/check-ui-geometry.ps1 -WindowTitle 添加应用` ✔）
- 旧门禁**钉住了上一版的计数标签尺寸** ✗（它当初是为了防止标签压到"确定"按钮 ✗）→ 已按**新布局**更新 ✔（新布局右侧留 8 px 间距 ✔，且几何检查实测 0 重叠 ✔）
- 新增门禁钉住：图标兜底链 ✔、主题参数与 `ApplyTheme` ✔、行自绘与空状态 ✔、占位提示 ✔、`BringToFront` ✔、调用方传入主题 ✔
- **未能在 200% 下复核** ✗：本机显示缩放现在是 **100%** ✔（上一轮记录过它是从 200% 变回来的 ✗）→ 高 DPI 下的观感需要你把缩放调回 125%/150%/200% 时再看一眼 ✔（代码已改成按行高比例，正是为了避免此类问题 ✔）

## 2026-09-11 「依旧很多应用没有 logo」——量出来之后，问题**不在我以为的地方**

你说"依旧很多应用没有 logo"，我先量了一遍**每个应用图标的来源**（而不是继续猜 ✔）：

```
PICKER ICONS total=95  catalogue=91  process=0  target=0  tile=3  elapsedMs=39
```

**添加应用对话框里 91/95 是真图标** ✔，只有 **3 个**（Steam、BOOTICE、微PE官网）落到生成的字母牌 ✔ —— 也就是说**这个列表本来就没问题** ✗。

### 真正没 logo 的是**工作流页那张卡**（我上一轮做的）

它的每一行只画了一个**状态圆点** ✗，**完全没有应用图标** ✗ —— 你看到的就是它 ✔。这是我的疏漏 ✔。

### 修法：**一处图标查找，两处共用**

把图标解析抽成共享的 `AppIcons`（`scripts/features/InstalledAppCatalog.cs` ✔）：目录已解析的图标 → 该进程可执行文件 → 启动目标 → 按名字生成的字母牌 ✔；带缓存 ✔（两处共用同一份，先打开哪个都会给另一个预热 ✔）。

- **工作流页每行现在显示应用真实 logo** ✔（截图可见 Chrome / Edge / Cursor 的真图标 ✔，只有 smoke 测试用的假应用落到字母牌 ✔）
- **添加应用对话框同样走这条链** ✔（行为不变，但实现不再重复 ✔）

### 顺带修掉一个我之前判断错的细节

我原以为"图标画不出来"✗，但实测里 **`process=0 target=0`** —— 兜底链的后两级**从来没被用到** ✗，说明图标不是取不到，而是**画在卡片上时看不见** ✗：不少应用图标是给白底/深底设计的、**带透明通道** ✗ → 直接画在卡片上就接近隐形 ✗✔。
**现在所有图标都画在一个浅色圆角底片上** ✔（深色主题下用深色底片 ✔），透明图标因此可见 ✔，观感也统一了 ✔。

### 噪音与验证

- 日志从"每次打开打 95 行"改成**只打兜底的那几行** + 一行汇总 ✔（95 行会把真正有问题的 3 行淹没 ✗）。
- 亮色/深色下对话框 **600×700、0 重叠 / 0 裁切** ✔；工作流页 **0 重叠 / 0 裁切** ✔；self-test ✔、validate ✔。
- 门禁已更新并钉住：共享 `AppIcons` 的签名与两级兜底 ✔、对话框改用共享查找 ✔、**工作流行必须带图标**（`AppIcons.For(card.ProcessName…` + `DrawTile(e.Graphics, rowIcon,` ✔）、底片绘制 ✔。

## 2026-09-11 设计审查开始执行：P0 两条（其中一条**我自己的发现是错的**，已撤回）

### ⚠️ P0-1 **撤回**：「结束界面预览」并不是给用户看的

我审查时从截图看到向导最后一步的主按钮写「结束界面预览」✗，判为 P0（用户会以为"刚才都只是预览"✗）。**动手前核对代码，发现我错了** ✗：

```csharp
private static string OnboardingNextButtonText(bool smokeMode, int currentStep)
{
    if (smokeMode) return ... ? "结束界面预览" : "预览下一任务";   // ← 仅测试模式
    return ... ? "打开首页" : "完成本步，继续";                    // ← 真实用户看到的
}
```

**真实用户看到的是「完成本步，继续 / 打开首页」** ✔ —— 文案本来就是对的 ✗，**P0-1 不成立，撤回** ✔。

**原因**：我的截图全部来自 `--ui-smoke` ✗ —— 测试模式与生产模式**文案不同** ✗。这条教训我记下了：**文案类审查不能只看 smoke 截图**，必须核对分支 ✔（本轮 P0-2 因此先在代码里确认它**不是** smoke 专属 ✔ 才动手 ✔）。

### ✅ P0-2 修好：设置页「按键来源保护」四处自相矛盾

**问题（实测截图 + 代码）**：

| 位置 | 原文 | 问题 |
| --- | --- | --- |
| 粗体行 | 「设备识别：**只有带 RC003 身份的事件可以执行遥控器动作**」 | **无条件**断言设备级隔离 ✗ |
| 复选框 | 画成**已勾选** ✔ | 看起来"已启用" ✗ |
| 徽章 | 「Raw Input **安全直通**」 | 说隔离**没生效** ✗ |
| 正文 | 「未安装签名通道时，言灵**不会拦截**来源未知的键」 | 与粗体行**直接矛盾** ✗ |

只读粗体那一行的用户会相信一个**不成立的保证** ✗ —— 这是本产品最需要诚实的一处 ✔。

**改法**：粗体行与复选框状态**都跟随真实状态** ✔（`exactDeviceIsolation` ✔）：

- 已隔离：「设备级隔离已启用：只有带 RC003 身份的事件会执行遥控器动作」✔（勾选 ✔）
- 未隔离：「尚未逐设备隔离：遥控器按键与实体键盘可能同时生效（签名通道为可选增强）」✔（**不勾选** ✔）

**验证**：重新截图已确认四处一致 ✔（粗体行→未勾选→徽章「Raw Input 安全直通」→正文 ✔）；`880×500` 深色下六页 **0 重叠 / 0 裁切** ✔；`validate` ✔、self-test ✔。
**门禁**：钉住两种状态的措辞 + 复选框状态随 `exactDeviceIsolation` 变化 ✔，并**禁止**退回无条件断言 ✔（旧门禁恰好钉住那句旧文案 ✗ → 已按新契约显式更新 ✔，不是绕过 ✔）。

### 下一步（继续按清单）

P1：字号规范（消除 7.x pt ✗）→ 页面标题与导航统一（4 处不一致 ✗）→ 琥珀一色一义 ✗ → 快捷键页按钮分级与危险动作收进「管理」✗。

## 2026-09-11 设计审查 P1 前三项：字号 / 页面标题 / 琥珀语义

### ① 字号：消除 7.x pt（13 处 → 0）✗→✔

实测 7.x pt 分布在：侧栏与向导左轨的品牌小字（7.1 / 7.3 / 7.4 ✗）、快捷键页状态与「固定稳定链路」（7.8 ✗）、语音页一行提示（7.8 **琥珀** ✗）、两处日志框（Consolas 7.8 ✗）、**自检页四行正文（7.9 ✗）**、以及一处自绘标签（7.6 ✗）。

**全部提到 8.0 pt** ✔。唯一保留的 7.x 是**遥控器插图上的「xiaomi」装饰字**（`Math.Max(5.2f, 7.2f * scale)` ✗）—— 它是图形的一部分而不是界面文字 ✔，已作为**明确记录的例外** ✔。

顺手改掉一处语义：语音页那行 7.8pt **琥珀色**说明文字（「返回、独立音量和开机键需由遥控器真实上报…」✗）→ 改 **muted 灰** ✔（琥珀只留给"需要你处理"✗）。

**验证**：字号变大最容易造成裁切 ✗ → 用几何检查实测：**100%（1280×840）与 880×500 深色下，六页全部 0 重叠 / 0 裁切** ✔（"no overlapping sibling controls and no clipped text" ✔）。
...
`ExpectedPageTitle` 同步 ✔（它被测试与截图脚本引用 ✔）。**门禁正确拦住**：两条门禁以 `AddPageTitle("按键"` 作为"当前生效页面"的判别标志 ✗ → 已按新契约**显式更新** ✔（不是绕过 ✔）。

### ③ 琥珀一色一义：完成不再是"警告色"✗→✔

向导左轨原来把**已完成**的步骤画成**琥珀色圆点 + 感叹号 `!`**，副标题写「进度已保存，**待复核**」✗ —— 用户走完一步看到的是"黄色警告"✗。现在：

- 已完成/已验证 → **绿色 ✓** ✔，副标题「**进度已保存**」✔（仍**只声明"已保存"**，不冒充"已验证" ✔ —— 应用内自测专门守着这条不变量 ✔，它比的是状态字符串，不受配色影响 ✔）
- **琥珀只保留"需要你处理"** ✔（向导最后一步的「仍有项目需要处理」✔）
- 副标题短了，左轨不再被两行文字撑高 ✔

**验证**：重新截图确认（第一步 = 绿色 ✓ + 「进度已保存」✔）。

### 门禁与交付

一条门禁同时钉住三件事 ✔：六个标题与导航一致 + 自测的四个标题分支一致 ✔、已完成步骤用 ✓ 与绿色 + 「进度已保存」✔、**7.9/7.8/7.6/7.4/7.3/7.1 pt 不得回归** ✔（插图上的 7.2 装饰字为记录在案的例外 ✔）。
...
## 2026-09-11 P1-5：「常用应用」与「应用工作流」合并为**一张卡**

### 先量后改：结论**推翻了我自己的假设** ✗

我原以为该"删掉工作流卡、只留常用应用"✗ —— 量了 `FavoriteAppsPanel`（322 行 ✔）后发现：**这张卡本来就实现了"每行一个随状态变化的主操作"** ✔（打开 / 设为当前 / 重新学习 ✔ + 删除 / 编辑 / 模式切换 ✔），也就是我建议给工作流卡的东西 ✗。所以**删它会丢功能** ✗（打开、编辑、删除、设为当前都会消失 ✗）。

而工作流卡（我上一轮加的 ✔）信息不同：它列出**配置里绑定但还没学会工作流**的应用 ✗ —— 那部分对老用户是真实信息 ✔，不能删 ✗。

### 改法：**一张卡，两段**

| 之前 ✗ | 现在 ✔ |
| --- | --- |
| 卡 A「常用应用」（用户自己的应用 + 丰富操作）+ 卡 B「应用工作流」（另一个列表）→ 用户要在两个列表之间自己对照"哪个才是我的"✗ | **一张卡**：上半 = 常用应用（用户自己的列表 ✔，操作一个没少 ✔）；**分隔线**下方 = 「应用工作流」次级分段（小一号标题 + 右侧汇总 + 一行一个待处理应用 ✔） |

- 删除了独立的 `BuildWorkflowStatusCard` ✔，其内容改为 `AddWorkflowStatusSection(card, …)` **画进同一张卡** ✔
- **所有原有能力保留** ✔（门禁专门钉住 打开 / 删除 / 编辑 / 设为当前 / 添加应用 五个回调都还在 ✔）
- 页面下方的提示行与滚动高度随卡片实际高度计算 ✔（不再是"两张卡高度相加" ✗）

**实测**：工作流页 1280×840 **0 重叠 / 0 裁切** ✔；可见文字结构 = 一张卡内的两段 ✔：
```
[常用应用] [＋ 添加应用] [还没有常用应用] …
──────────────
[应用工作流] [4 个应用：0 个已就绪 · 3 个还没学习] [还没学习] …（一行一个）
```

### 过程中被门禁拦下三次（都正确 ✔）

1. 我新写的门禁用 `!app.includes("BuildWorkflowStatusCard")` 判"旧方法已删" ✗ → 被**我自己上一轮留下的过时注释**判失败 ✗（注释里引用了一个已不存在的方法 ✗）→ 改注释 ✔
2. 上一轮的门禁钉住 `BuildWorkflowStatusCard` 这个**方法名** ✗ → 按新结构显式更新（契约不变：工作流行只在工作流页 ✔）
3. 另一条门禁钉住 `BuildFavoriteAppsCard(content, 100);` 这个**旧签名** ✗ → 更新为新签名 ✔

**同一条教训第二次出现**：判据要匹配**代码**，不要匹配**散文/注释** ✗（上轮已记过一次 ✔，这次是我自己的注释又踩了 ✔）。

## 2026-09-11 P1 最后一项：「添加应用」少一步，并且**提示不再说谎**

### 量清楚了真实流程（不是凭印象 ✗）

```
① 点「＋ 添加应用」→ 选应用（选择器 ✔，会自动把它启动并切到前台 ✔）
② 在那个应用里点一下输入框（学习 ✔，之后面板出现「保存」横幅 ✔）
③ 点「保存」
④ 再点「设为当前」   ← 这一步才是问题所在
```
...
> 「**保存成功：以后按住录音键，文字都会进入 X 的输入框**」✗

但如果不点第 ④ 步「设为当前」✗，文字**可能仍进入别的应用** ✗ —— 提示**承诺了当时并不成立的行为** ✗。这和前面 P0-2（设置页四处矛盾 ✗）是同一类问题 ✔：**文案先说，行为没跟上** ✔。

### 改法：让行为追上文案（顺带少一步 ✔）

保存成功后：**如果这个应用还不是当前应用，就把它设为当前** ✔，
- 提示随之改为 **「已添加并设为当前：以后按住录音键，文字都会进入 X 的输入框」** ✔（**说的是刚刚发生的事实** ✔）
- 已经是当前应用时 → 提示为「已保存：…」✔（不谎称"已设为当前" ✔）
- 保存失败 → 提示不变 ✔

**没有做**的另一件事（**故意的** ✗）：**不自动保存** ✗ —— 产品有"**必须先真实学习并验证输入框才能保存**"的既有约束 ✔（这也是防误发的核心保护 ✔），自动保存会破坏它 ✗，所以那一步必须由用户点 ✔。

### 验证：纯函数 + 自测 + **负对照**

真实路径需要"在真应用里点一下输入框"✗ —— 本机无法供应 ✗（**如实说明** ✔）。所以把判定与文案抽成纯函数（`ShouldMakeCurrentAfterSave` / `FavoriteSaveMessage` ✔），由 host 自测钉住 ✔，并做了**负对照**：

```
正确实现      → self-test passed
故意写成 return saved;（✗）→ self-test failed:
   "Saving an application's input box no longer decides whether to make it current"
恢复          → self-test passed
```

**门禁**同步更新 ✔：原门禁钉住旧提示文案 `保存成功：以后按住录音键` ✗ → 按新契约显式更新 ✔（并补钉判定本体 `return saved && !alreadyCurrent;` ✔、新文案 ✔、日志字段 `madeCurrent=` ✔、两条自测失败信息 ✔）。

## 2026-09-11 P1 收尾：术语中文化（界面里的英文术语**首次出现处给中文解释** + 设置页术语表）

### 先量：界面里到底有多少英文术语

用"含该术语的字符串字面量"逐个查 ✔（不是凭印象 ✗）。结论：**界面里其实很少** ✔ —— 绝大多数命中都是**自测断言与日志** ✗（`Browser Remote Lite apply changed protected state…` 之类 ✗），真正给用户看的只有：

| 术语 | 出现处 |
| --- | --- |
| `Profile` | 快捷键页卡片副标题（**首次出现处** ✔） |
| `Smart Profiles` | 向导第 5 步的勾选项 + 说明行 |
| `Browser Remote Lite` | 向导第 5 步的按钮 |
| `Raw Input 安全直通` | 设置页徽章 / 自检页 / 高级诊断 |

### 改法

**① 首次出现处加中文解释** ✔（就地解释，不打断阅读）：

- 快捷键页副标题：「…；**快捷键 Profile（应用专属的按键方案）**不会修改语音链路」✔
- 向导：「配置**浏览器遥控（Browser Remote Lite）**」✔
- 向导：「是否启用 **Smart Profiles（按应用自动切换键位方案 · 可选，默认关闭）**」✔

**② 设置页新增「术语表」卡** ✔（六条，一文一义 ✔）：
快捷键 Profile / Smart Profiles / Browser Remote Lite / Raw Input / Live HUD / Context Deck ✔ —— 用户不用在六个页面里找解释 ✔。

**③ 我判断"不需要改"的一处**（并说明理由 ✔）：`Raw Input 安全直通` 这个**徽章**保持简短 ✗ —— 它旁边的正文已经用中文解释了含义 ✔（「未安装签名通道时，言灵不会拦截来源未知的键…」✔），而术语表里也有一条 ✔。**徽章塞解释会破坏它的功能**（一眼看状态 ✗）✔。

### 验证

- 文字判据（可见控件）：术语表六条**全部渲染** ✔，页面底部可见 ✔
- 几何：`1280×1400`（把窗口加高让整页可见 ✔）下**无重叠、无裁切** ✔
- `validate` ✔、self-test ✔
- 门禁：原先钉住向导旧文案 `配置 Browser Remote Lite` / `是否启用 Smart Profiles` ✗ 的门禁**按新文案显式更新** ✔；并新增门禁钉住三处解释 + 术语表标题 + 两条术语正文 ✔

### 顺带记一个测量小技巧

设置页现在超出一屏 ✗，截图默认看不到底部 ✗。用几何检查的 `-ForceSize 1280x1400`（屏幕工作区 1440 装得下 ✔）**把窗口加高**，整页一次拍全 ✔ —— 比"发滚动消息"可靠得多 ✔。

## 2026-09-11 P2-1：向导加了「稍后再说」—— 而且**驱动测试抓到一个会关掉整个应用的 bug**

### 问题比你之前提的更严重 ✗

我原以为只是"缺一个跳过按钮"✗。读了才发现：**「完成本步，继续」本身就是一堵墙** ✗ ——

- 第 1 步（遥控器）：不检测到**真实的遥控器方向键**就不前进 ✗
- 第 2 步（音频）：不同时检测到 **CABLE Input + CABLE Output + RC003 麦克风**就不前进 ✗
- 第 3 步（语音工具）：同样有证据门槛 ✗

→ **手上没遥控器、或没装 VB-CABLE 的用户，会被卡在向导里，唯一出路是点窗口的关闭按钮** ✗（而且那样也说不清"设置到底完成没有"✗）。

### 改法（刻意选最小且诚实的方案 ✔）

**「稍后再说」= 关闭向导，进度原地不动** ✔

- 下次打开向导会**停在同一个任务** ✔ —— 这本身就是"未完成提醒" ✔
- **不给"跳过"造一个新状态** ✔：我先考虑过"标记为跳过然后前进" ✗，但发现向导的规则是**任何早于当前步的步骤都显示为 `saved`（已保存）** ✗ —— 我上一轮刚把 `saved` 改成绿 ✓ ✔，所以"跳过的步骤"会被显示成**已完成** ✗✗（正是这个产品一直在防的"把未验证说成已完成"✗）。改成"关闭而不前进"就完全绕开了这个陷阱 ✔
- 最后一步不显示该按钮 ✔（在那里它等同于关窗 ✗）

### ⚠️ 驱动测试抓到的真 bug（代码审查没看出来 ✗）

写完先跑探针：打开向导 → 点「稍后再说」 → 检查"向导关了吗、主窗口还在吗" ✔：

```
  the wizard is open
  found 稍后再说; pressing it
  FAIL: the main window disappeared with the wizard      ← 第一次运行
```

**原因**：处理逻辑写在 `ShowSetupWizard`（**VibeMicForm 的方法**）里的闭包中 ✗ → 未限定的 `Close()` 关的是**主窗体** ✗，于是**整个应用被关掉** ✗。
**修法**：`wizard.Close();` ✔（限定到向导本身 ✔）。修后：

```
  PASS: 稍后再说 closed the wizard and left the application running
  日志: ONBOARDING later=true step=0
```

**教训**：这个 bug 读代码时看不出来 ✗（`Close()` 在那个上下文里"看起来"很自然 ✗）—— **只有真实驱动才会暴露它** ✔。门禁里专门钉住 `wizard.Close();` ✔ 与那句注释 ✔，防止退回。

### 过程中的一个乌龙（也记下来 ✔）

我第一次插入代码时**插错了方法** ✗：文件里有三个向导（`ShowSetupWizard` / `ShowSetupWizardElevenStepLegacy` / `ShowSetupWizardLegacy` ✗），而 `footer.Controls.Add(back)` 与 `back.Click += …` 在**多个向导里文本完全相同** ✗ → 我的字符串替换命中了旧版 ✗，编译报"找不到 setupLater"✗。修法：**先按方法边界确认行号** ✔，再按行号插入 ✔，最后逐条验证 `setupLater` 的 5 处引用**全部落在 `ShowSetupWizard` 内** ✔。

## 2026-09-11 P2-2：界面主题改成**分段选择**，不再是"三个命令按钮"

**问题**（我审查时看出来的 ✗）：设置页的 `白天模式 / 夜间模式 / 跟随 Windows` 三个按钮里，**当前选中的那个画成实心紫按钮** ✗ —— 和"点它就会切过去"的按钮**长得完全一样** ✗ → 用户看不出"哪个是当前状态"✗，会以为它是一个动作按钮 ✗。

**改法**：按分段控件的语义重画 ✔

| | 之前 ✗ | 现在 ✔ |
| --- | --- | --- |
| 选中项 | 实心紫底 + 白字（像命令 ✗） | **浅紫底 + 紫边框 + 紫字 + `●`** ✔ |
| 其他项 | 浅底 + 深字（无区分 ✗） | 普通底 + `○` ✔ |

深色主题下用浅紫强调色 `(150,135,255)` ✔ 与浅紫底 `(52,47,88)` ✔，保持可读 ✔。同时删掉了原先那段重复的着色代码 ✗（新 helper 已经覆盖 ✔）。

**验证**（三条互相独立的判据 ✔）：
- **文字判据**：`[●  白天模式] [○  夜间模式] [○  跟随 Windows]` ✔
- **几何**：`880×500` 与 `1280×1400` 下**无重叠、无裁切** ✔（点号前缀**没有撑破**按钮 ✔ —— 这是我加 `●` 时最担心的 ✗）
- **目视**：裁剪该行确认「● 白天模式」为浅紫底+紫边框+紫字 ✔，另两个为空心点+普通底 ✔（读起来是"选择"而不是"命令" ✔）

门禁钉住分段控件的签名、`●`/`○` 前缀、选中底色与边框色 ✔，以及三处调用点 ✔。

## 2026-09-11 P2-3：自检行默认**折叠成一行**（并把开发者错误码收进详情）

### 问题（审查所记）

自检每项都是 **4 行** ✗（正确状态 / 当前状态 / **原因 [VF-COMPONENTS]** / 下一步 ✗）→ 一屏只看得到约 **3 项** ✗，而且**开发者错误码**挡在结论前面 ✗。

### 改法

| | 之前 ✗ | 现在 ✔ |
| --- | --- | --- |
| 每行高度 | 104 px（4 行） | **64 px**（1 行）✔ |
| 行内内容 | 正确状态 / 当前状态 / 原因 [VF-…] / 下一步 | **状态 + 标题 + 当前状态 + 「详情」+ 动作按钮** ✔ |
| 细节点开 | — | 正确状态 / **原因 [VF-…]** / 下一步 ✔ + 「收起详情」 |
| 页面副标题 | 「逐项说明正确状态、当前状态、原因和修复入口」✗（已不准确 ✗） | 「每项给出结论与修复入口；点「详情」看正确状态、原因与下一步」✔ |

- **一屏多出约 60%** 的行 ✔（104 → 64 px ✔）
- 展开状态存在**字段**里 ✔（页面每次导航都重建 ✗，所以不能存在控件上 ✗）
- 卡片高度与行定位改为**按行高求和** ✔（不再是 `行数 × 112` ✗）

### ⚠️ 过程中发现并修掉的一个真问题：**折叠开关原本键盘用不了** ✗

我第一版把「详情」做成 **Label** ✗（下划线 + 手型光标，看起来像链接 ✔）。两个后果：

1. **不可聚焦** ✗ → **键盘用户无法展开详情** ✗（只有鼠标能用 ✗）
2. 探针的 `BM_CLICK` 对 Label **无效** ✗ → 我的驱动验证第一次显示"点了没反应" ✗

**修法**：改成**按钮样式的链接** ✔（`FlatStyle.Flat` + 无边框 + 透明底 + 下划线 + 强调色 ✔）→ 外观不变 ✔，但**可 Tab 可达** ✔、驱动也能点 ✔。

### 验证（两条独立判据）

- **驱动**：点「详情」→ 展开并出现「收起详情」+「正确状态：主程序、语音捕获…版本一致…」+「**原因 [VF-COMPONENTS]**：…」✔（错误码确实藏在折叠里 ✔）
- **键盘走查**：自检页可达控件 **20 → 30** ✔（正好 +10 = 十个「详情」按钮 ✔），且走查里**包含「详情」**✔、负对照 `visitedNonTabStop=0` ✔
- **几何**：`880×500` 与 `1280×1400` 下均无重叠、无裁切 ✔

## 2026-09-11 P2-4 / P2-7：次要动作不再装成主按钮；列表一屏多两行

### ① 「试听结束提示音」不再是整条淡紫宽按钮 ✗→✔

**问题**：交互反馈卡片里这个**次要动作**占了 **284 px** 的淡紫宽条 ✗ —— 与卡片里真正的主按钮**抢视觉** ✗（用户会以为它是这一块的主操作 ✗）。
**改法**：按文字宽度收到 **152 px** ✔（并注明原因 ✔）。几何实测无裁切 ✔。

### ② 添加应用：行高 54 → 44 ✗→✔

**问题**：本机 **95 个应用** ✔，54 px 行高下一屏只有 **8 行** ✗ → 要滚很多屏 ✗。
**改法**：行高 **44 px** ✔ → 一屏 **10 行** ✔（+25% ✔）。
**关键点**：行的内部布局（图标尺寸、文字位置）本来就是**按行高比例**算的 ✔（这是我上次重做对话框时特意改的 ✔），所以降低行高**不需要改任何绘制代码** ✔，也不会挤在一起 ✔ —— 截图确认图标、名称、状态都在位 ✔。

**实测**：对话框 600×700、**0 重叠 / 0 裁切** ✔；设置页在 `880×500` 与 `1280×1400` 下同样 0/0 ✔。

### 顺带发现（记录，不在本轮改 ✗）

列表里出现 `Git FAQs (Frequently Asked Questions)` ✗ —— 这类"说明文档"型条目**不该作为输入目标** ✗。现有 skip 名单只含 `readme / help / documentation` ✗，不含 `faq`/`frequently asked` ✗。**建议后续补进名单** ✔（改动一行 ✔，但属于"应用筛选"那条线 ✗，不塞进本轮 ✔）。

## 2026-09-11 P2-5：新增**界面缩放**设置（100% / 110% / 125%）

### 为什么做这个

审查里我把它标为"比重做配色更能解决看不清"✗：正文主体在 8–9.5 pt ✗，而很多用户的 Windows 显示缩放他们自己不方便改 ✗。现在应用内多了一档自己的缩放 ✔。

### 实现

- **配置**：`uiScalePercent`（0/缺失 = 100% ✔），夹取在 **100–150%** ✔
- **生效点**：`UiDisplayScale.UserScale` ✔，并**同时**乘进两处缩放来源（`UiDisplayScale.ForControl` ✔ 与主窗体的 `DesignScale()` ✔）—— 只改一处会得到"页面放大但窗口没放大"的错位 ✗，所以门禁**同时钉住两处** ✔
- **验证专用参数**：`--ui-scale <percent>` ✔（与既有 `--ui-theme` 同一模式 ✔，正常使用不会用到 ✔）
- **设置入口**：设置页底部新增「界面缩放」卡 ✔，分段控件与主题同样是 `● / ○` 语义 ✔
- **诚实说明**：窗口与布局是**加载时定尺寸**的 ✗，所以设置卡与提示都写明「**改动在重启言灵后生效**」✔ —— 我没有假装它能即时重排（即时重排需要重算窗口、边距与每个页面 ✗，风险高 ✗）

### 验证（三段独立证据 ✔）

| 判据 | 结果 |
| --- | --- |
| **文字**（可见控件） | `[●  100%] [○  110%] [○  125%]` + 「改动在重启言灵后生效」✔ |
| **驱动点击** 125% | 提示「界面缩放已设为 125%，重启言灵后生效」✔ → 重绘为 `● 125%` ✔ → 日志 `UI SCALE set=125 saved=True` ✔ |
| **实际效果**（`--ui-scale 125` 启动） | 启动日志 `override=125 configured=0 factor=1.25` ✔；窗口 **1600×1050**（= 1280×840 × 1.25 ✔）、最小 1100×625（= 880×500 × 1.25 ✔）；几何检查 **无重叠 / 无裁切** ✔ |

### 过程中两个"仪器"教训（都记下来了 ✔）

1. **等待时间不够会误判** ✗：我第一次在启动后 **10 秒**测量，得到"缩放没生效"✗；应用自己在 14 秒时的日志明确显示 `window=1600x1050` ✔ —— 是**我测早了** ✔。
2. **几何检查脚本的 `-ProcessId + -WindowTitle 言灵` 模式会抓错窗口** ✗：它量到一个 **416×217** 的隐藏辅助窗口 ✗（标题也以「言灵」开头 ✗）。用 `-ExeArguments` 那条路径（脚本自己启动并认主窗口 ✔）测得的 1600×1050 才是对的 ✔。**建议后续给该模式加"取面积最大的可见窗口"**✔。

## 2026-09-11 两个小项：文档类条目不再作为输入目标；几何检查不再量错窗口

### ① skip 名单补 `faq` / `frequently asked`

**依据**：本机 **shell:AppsFolder** 里有 4 条文档类条目 ✔（`Inno Setup Documentation`、`Inno Setup Documentation (Dark)`、**`Inno Setup FAQ`**、**`Git FAQs (Frequently Asked Questions)`**✗）—— 而 `documentation` 已在 skip 名单里 ✔、`faq` 不在 ✗ → **名单的判定取决于安装包用了哪个词** ✗。

**改后实测**：候选数 **95 → 93** ✔（正好去掉那两条 ✔）；几何与对话框测量不变（600×700、0/0 ✔）。

**我自己的预期错了** ✗：我预测只少 **1** 条 ✗，因为我用来预测的扫描**只覆盖了开始菜单快捷方式** ✗，漏掉了 shell 命名空间 ✗ —— 而目录枚举**两者都遍历** ✔。**错的是我的预期，不是代码** ✔（已写进文档与门禁注释 ✔）。

### ② 几何检查脚本会量到**别的进程**的窗口

**问题**（本轮验证界面缩放时发现 ✗）：`-ProcessId + -WindowTitle 言灵` 报 **416×217** ✗，而应用自报 **1600×1050** ✔。读代码发现三个叠加的错误 ✗：
1. **没有按进程过滤** ✗（会匹配任何进程里标题以「言灵」开头的窗口 ✗）
2. **取第一个匹配就停** ✗（而不是取主窗口 ✗）
3. 只枚举顶层窗口 ✗（且有一段空 `foreach` 死代码 ✗）

主窗口搜索（`Find-TopWindow`）也有"最后枚举到的赢"✗ 的问题 ✔。

**改法**：两处都改为 **按进程过滤 + 取面积最大的匹配窗口** ✔，并删掉那段死代码 ✔。
**验证**：同一命令现在报 **1600×1050** ✔（与应用自报一致 ✔）、0/0 ✔；对话框测量（添加应用 600×700）仍正常 ✔。

**顺带发现**：临时驱动脚本 `vibe-dialog-sweep3.ps1` 的 rename 用例现在报"找不到重命名" ✗ —— 因为 P1-4 已把重命名移进「管理」菜单 ✔，属**预期**结果 ✔（该临时脚本待更新 ✗；菜单路径已由 `vibe-manage-menu-probe.ps1` 覆盖 ✔）。

## 2026-09-11 P2-6 收尾：审查建议的"搜索"**已经存在**，我判断不需要再做全局搜索（并说明一处我没能驱动的验证）

### 量出来的事实

审查最后一条建议是「全局 Ctrl+F 式搜索」✔。我先量"哪里真的需要搜索" ✗：

- **动作选择器**（快捷键页点某个键的动作时打开 ✔）**本来就有搜索框** ✔：`搜索动作` 标签 + 输入框 ✔，`refresh()` 按 `choice.Label` **实时过滤** ✔（大小写不敏感 ✔），并且过滤后**自动选回当前动作** ✔ —— 这里有全应用最长的列表（所有快捷键动作 ✔），而它有搜索 ✔
- **添加应用**对话框：本轮之前已加过搜索 ✔（而且我用真实驱动验证过 ✔：`chrome` → 「显示 1 / 95」✔）
- **其余页面**：控件最多 13 个 ✗（快捷键页 ✔）→ **不需要搜索** ✔

**结论（我的设计判断 ✔）**：不再加"应用级搜索浮层" ✗ —— 真正长的列表已经有了 ✔，其余页面短到不值得 ✗。这条判断写进门禁注释与本文档 ✔（不是默默跳过 ✔）。

### ⚠️ 一处我**没能**驱动的验证（如实说明 ✗）

我写探针想**端到端驱动**这个对话框 ✗（打开选择器 → 在搜索框输入 → 数列表项数 ✔）。结果失败 ✗，追查原因如下（**是我的探针点错了控件** ✗，不是产品问题 ✗）：

- 我点的 24 个 `▶` 按钮，其实是行内的**「测试」按钮** ✗（`layerTest.Click → TestGestureLayer` ✔），**不是**打开选择器的入口 ✗
- 选择器的真正入口是行内那个**动作名控件**（`layerEdit` ✔），而它被 `hardwareReady` 门控 ✗（`layerEdit.Enabled = hardwareReady` ✔），smoke 下多半禁用 ✗ → 点击无效 ✗

**所以我对这一项的验证边界是**：代码层面已确认实现 ✔ + **同一套机制**（对话框内实时过滤）在添加应用对话框上**已被端到端驱动验证** ✔ —— 但**这个具体对话框我没有端到端驱动过** ✗，不能把"同机制已验证"当成"这一个也验证过" ✗。需要真机（遥控器就绪）时才能补上 ✔。

## 2026-09-11 设计审查**收尾复核**：P0 / P1 / P2 全部处理完毕

| 项 | 结论 | 验证方式 |
| --- | --- | --- |
| **P0-1** 向导末尾文案「结束界面预览」 | **撤回** ✗ —— 该文案只在 `--ui-smoke` 下出现 ✔，真实用户看到的是「完成本步，继续 / 打开首页」✔ | 读代码分支 ✔ |
| **P0-2** 设置页「按键来源保护」自相矛盾 | **已修** ✔ 粗体行与复选框都跟随真实状态 ✔（未隔离 = 不勾选 + 说明原因 ✔） | 截图四要素一致 ✔ + 880×500 深色 0/0 ✔ |
| **P1-1** 字号 7.x pt | **已修** ✔ 13 处全提到 8.0pt ✔（仅遥控器插图的装饰字保留 ✔） | 100% 与 880×500 深色六页 0 重叠/0 裁切 ✔ |
| **P1-2** 页面标题与导航不一致 | **已修** ✔ 4 处统一（快捷键/语音/自检/设置 ✔） | 源码 + 走查 ✔ |
| **P1-3** 琥珀一色两义 | **已修** ✔ 已完成步骤改绿色 ✓ + 「进度已保存」✔，琥珀只留"需处理" ✔ | 向导截图 ✔ |
| **P1-4** 快捷键页 7 个等权按钮 | **已修** ✔ 收成 `切换` + `管理` 菜单（危险动作置底 ✔） | **键盘驱动菜单** → 打开重命名对话框 ✔ |
| **P1-5** 两张工作流卡 | **已合并** ✔ 一张卡两段（常用应用 + 待处理工作流 ✔） | 文字结构 + 几何 ✔；功能零丢失（门禁钉住 5 个回调 ✔） |
| **P1-6** 「添加应用」步数 | **少一步** ✔ 保存即设为当前 ✔，且**修掉了"提示与事实不符"** ✗→✔ | 纯函数自测 + **负对照**（故意写错 → 自测失败 ✔） |
| **P1-7** 术语无中文解释 | **已修** ✔ 三处首次出现加中文 ✔ + 设置页新增「术语表」六条 ✔ | 文字判据 + 1280×1400 几何 ✔ |
| **P2-1** 向导无出路（硬件未就绪走不下去 ✗） | **已修** ✔ 新增「稍后再说」＝关闭向导、进度不动 ✔ | **驱动探针 PASS** ✔（并抓到 `Close()` 误关主窗体的 bug ✗→✔） |
| **P2-2** 主题控件像命令按钮 | **已修** ✔ 分段控件（● 选中 / ○ 未选 ✔） | 文字 + 几何 + 目视 ✔ |
| **P2-3** 自检行 4 行 + 错误码 | **已修** ✔ 默认 1 行 64px、详情可展开 ✔（一屏约 +60% ✔） | 驱动展开 ✔ + **键盘走查 20→30** ✔ |
| **P2-4** 列表一屏 8 行 | **已修** ✔ 行高 54→44 → 一屏 10 行 ✔ | 截图 + 600×700 几何 ✔ |
| **P2-5** 无界面缩放 | **新增** ✔ 100/110/125% ✔（重启生效 ✔） | 驱动设值 ✔ + `--ui-scale 125` → 窗口 **1600×1050** ✔ |
| **P2-6** 全局搜索 | **判断为已满足，不做** ✔ 动作选择器本就有实时搜索 ✔；其余页面 ≤13 控件 ✗ | 代码 ✔；**该对话框未能端到端驱动** ✗（见下） |
| **P2-7** 次要按钮抢视觉 | **已修** ✔ 284px → 152px ✔ | 几何 ✔ |
| 追加 | skip 名单补 `faq` ✔（95→93 ✔）；几何检查改取**最大可见窗口** ✔（1600×1050 ✔） | 实测 ✔ |

### 本次收尾时明确**没有**声称验证的东西（✗ 如实列出）

1. **动作选择器的搜索**：实现已确认 ✔，但**没能端到端驱动** ✗ —— 它的入口控件被 `hardwareReady` 门控（smoke 下禁用 ✗），我最初点的 24 个 `▶` 其实是「测试」按钮 ✗。**同机制**在添加应用对话框上已被驱动验证 ✔ —— 但那是**不同的对话框** ✗，不能互相代替 ✔。
2. **200% / 高 DPI 下的新界面**（本轮多数改动在 100% 下验证 ✔）：本机缩放是 100% ✗，改为 125%/150%/200% 后的观感仍需真机复核 ✗（尤其新加的**界面缩放**与**自检折叠行** ✔）。
3. **真机链路**：录音键、RC003、VB-CABLE 相关的一切 ✗（本会话只做界面 ✔）。
4. **签名 / 干净账户安装 / 无 CABLE 首启 / CI** ✗ —— 见发布清单 ✔。

### 交付状态

`validate` ✔、self-test ✔（含负对照 ✔）、发布链 ✔、12 例矩阵 ✔、**安装器 exit 0、51/51 逐文件一致** ✔、冻结采集件哈希未变 ✔、工作区无未提交改动 ✔。

## 2026-09-11 工作流页三处问题（你报的）—— 从根上修，不是改样式

### 先看再改（三处都先量/先看 ✔）

**① logo 大量缺失** ✗ —— 根因不是"图标取不到" ✗：这些行**只问"正在运行的进程"要可执行文件** ✗，而"已安装但没运行"的应用**没有进程可问** ✗ → 于是绝大多数行落到**字母牌** ✗，**尽管目录里本来就有真图标** ✔（本机 95 条中 91 条有 ✔）。
**修法**：新增 `InstalledAppCatalog.IconForProcess(processName)` ✔（复用已有的目录缓存 ✔，并顺带把图标一起缓存 ✔），行内**优先查目录** ✔，字母牌只作最后兜底 ✔。
**验证（目视）**：`cursor` / `chrome` / `msedge` 现在都是**自己的真 logo** ✔；只有 smoke 测试的假应用仍是字母牌 ✔（正确 ✔）。

**② "乱码" + 下面那句不需要的「工作流」文案** ✗ —— 两件事 ✔：
- 「乱码」是 `SectionTitle` 的**图标字形**（`\uE71D`）✗，在标题前渲染成一个**小而无意义的方块** ✗ → 已改为**纯文字标题** ✔
- 标题原本叫「**应用工作流**」✗（功能名 ✗），下面的行还带一句「**键位 … · 工作流 未设置 · 语音工具 …**」✗（用配置文件的词汇重复状态 ✗）→ 标题改为「**还需要处理**」✔，**整句删除** ✔，行内只剩 **图标 + 名称 + 状态 + 一个动作** ✔

**③ 两段左边缘没对齐** ✗ —— 量出真实数字 ✔：常用应用面板的内容在 **面板 26 / 卡片 44** ✔、它的行内容在 **面板 40 / 84** ✔；而我的小标题在卡片 **26** ✗（差 18px ✗）、行的图标在卡片 **22** ✗（差 36px ✗）。
**修法**：小标题 → **44** ✔、行图标与文字 → 与面板同列 **40 / 84** ✔。
**顺带被工具抓到的回归** ✗：小标题右移到 44 后**撞上右边 240 处的汇总标签**（4px ✗）→ 几何检查立刻报 `overlaps=1` ✔ → 把标题宽度从 200 收到 **190** ✔ → 回到 **0 重叠 / 0 裁切** ✔✔

### 验证

- **目视**（亮色裁剪）：三个标题同一条左边缘 ✔；`cursor/chrome/msedge` 真图标 ✔；行内只有名称+状态+动作 ✔
- **文字判据**：页面文字里已无「应用工作流」标题 ✗、无「键位 … · 语音工具 …」长句 ✗
- **几何**：`1280×840` **0 重叠 / 0 裁切** ✔（含修掉的 4px 回归 ✔）；`880×500` 亦为 0/0 ✔
- 门禁钉住：目录图标查询 ✔、纯文字标题与「还需要处理」✔、44/190 与 40/84 的对齐坐标 ✔，并**禁止**那句长文案与带字形的标题回来 ✔

### 一次我自己的流程失误（第三次同类，这次改了做法 ✔）

修这三处的过程中，我**两次在 `validate` 失败的情况下完成了提交** ✗ —— 因为我把命令串成 `node validate.js; git commit` ✗，失败没有中止后续 ✗。第一次是**自检页副标题**被旧门禁钉住 ✗，这轮是**两条旧门禁**（工作流行的旧调用 `AppIcons.For(..., null, ...)` ✗、以及被我改名的标题字符串 `"应用工作流"` ✗）。

**已改做法**：提交前把 `validate` 的退出码**当作闸门** ✔（`if($LASTEXITCODE -eq 0){ git commit … }` ✔），本轮后半程就是这样做的 ✔ —— 一次被挡住、修正后才提交 ✔。

**顺手记下的门禁设计教训** ✗：**字符串钉住会在每次行为改动时过期** ✗。本轮接连撞到 4 条 ✗（自检副标题、`checksHeight` 算法、工作流行调用、标题文案 ✗）。它们的**契约仍然正确** ✔，失效的只是**措辞/方法名** ✗ —— 所以更新门禁时要**保留契约、只改字面量** ✔（我这几轮都是这样做的 ✔），并且尽量钉**行为**（如"两处缩放来源都乘了用户缩放"✗）而不是**文案** ✔。

## 2026-09-12 RC003 **电源键**支持：诊断完成（含 MiVibe-Remote 的机制与我们的真实差距）

### 一、MiVibe-Remote 怎么做到的（读它的源码，不是看宣传 ✔）

`tools/keyboard-remap/Enable-SplitVoiceRemap.ps1` 把答案写死在注释里 ✔：

```
HKLM\SYSTEM\CurrentControlSet\Control\Keyboard Layout → "Scancode Map" (REG_BINARY)
  00 00 00 00 | 00 00 00 00 | 03 00 00 00
  35 E0 5E E0     ← 遥控器电源键 E0 5E → Numpad Divide
  64 00 3F 00     ← F5 → F13（"安静的接收端"）
  00 00 00 00
```

要点（都是它自己的注释/README 说的 ✔）：
- **RC003 电源键在 Windows 上就是键盘扫描码 `E0 5E`** ✔（HID Power usage 被键盘类驱动映射成标准键事件 ✔）
- 它用**系统级扫描码映射**把该键变成应用能用的普通键 ✔ → **需要管理员 + 重启 Windows** ✔
- 该脚本**写前读回校验** ✔、**已有别人的映射就拒绝修改** ✔、并**提供 Disable 脚本还原** ✔ —— 这三条安全设计值得照抄 ✔
- 副作用它写明了 ✔：**F5→F13 是全局的**（所有键盘的 F5 都变 F13 ✔）
- 菜单/返回/音量键走的是另一套（`src/MiVibe.Remote.KeyBridge/`，Python + hid_tap ✔，**需要管理员与三段校准** ✔，README 说它**不安装 HID 过滤驱动** ✔）

### 二、我们这边为什么"收不到"（代码实测，结论与预期相反 ✗）

**不是看不见** ✗ —— 我们的桥**早就识别电源键** ✔：

```csharp
// VoxDeckInputBridge.cs:4112-4113
// (VK 0xFF / scan 0x5E) across Bluetooth reconnects.
return vk == 0x74 || vk == 0xF5 || (vk == 0xFF && scan == 0x5E);
```
- 过滤掩码里也已有 `mask[0x5E] = 1;` ✔（`:558` ✔），并在 `:1207` 的校验里被断言 ✔
- 但 `HID_USAGE_POWER = 0x66` ✔ 这个常量**声明后从未使用** ✗（全文件仅此一处 ✗）
- 而**没有任何 mapping 以 vk=0xFF / scan=0x5E 存在** ✗（配置里的键名是 上键/下键/左键/右键/确认键/Home/TV/音量±/功能键短长 ✗），**界面也没有对应行** ✗

→ 所以真实情况是：**按键被识别了，但没有映射与入口去"用它"** ✗，同时我们的 `docs/QUICK_START_ZH.md` 还写着「开机、返回和独立音量键**没有稳定 Windows 事件**，V2.0 不提供映射」✗ —— **这句话对电源键是错的** ✗，必须更正 ✔。

### 三、下一轮的确切改法（已在代码里定位到落点 ✔）

映射结构是齐全的 ✔：`ShortcutMapping { name, label, vk, scan, enabled, suppress, mode, shortcut, shortShortcut, longShortcut, doubleShortcut, longPressMs, sourceType, usagePage, usage }` ✔ —— 连 HID 级的 `usagePage/usage` 都有 ✔。因此：

1. **加一条内置映射**：`电源键` ✔ → `vk = 0xFF` ✔ / `scan = 0x5E` ✔（并按需带 `usagePage/usage` ✔）
2. **快捷键页加一行**（放在「录音键」附近 ✔），动作由既有的动作选择器挑 ✔（选择器是通用的 ✔，无需新增 ✔）
3. **`suppress` 语义要用上** ✔：映射后可让钩子吞掉该键 ✔，使 Windows 不再按"电源键"处理 ✔（我们现在是安全直通 ✔，钩子确实在链路上 ✔ → 这条可行 ✔）
4. **只有当用户机器上仍被 Windows 抢先处理时**才提供 MiVibe 那种**系统级扫描码映射** ✗ —— 按它的安全设计做（管理员 ✔、写前校验 ✔、拒绝覆盖他人映射 ✔、可一键还原 ✔、明确要求重启 ✔），并且**默认关闭** ✗
5. **更正文档** ✗：「开机键没有稳定事件」这句话改为事实描述 ✔（`E0 5E` 可识别 ✔，能否直接使用取决于 Windows 是否抢先处理 ✔）

### 四、只有你能做的验证（如实说明 ✗）

**按键必须由你按** ✔。我能在代码与日志两侧都准备好（把 vk/scan 与 `suppress` 生效写进运行时日志 ✔），然后你按一次电源键，我从日志确认它是否被识别、是否被吞掉、动作是否执行 ✔ —— 在此之前我不会声称"电源键已可用"✗。

## 2026-09-12 电源键：**能改，但比看起来大**；我本轮改了又**干净回退**（含两次失误的记录）

### 一、我实际做到了什么（有真实证据 ✔，但**未提交**✗）

我按上一轮的计划改了**产品代码**并跑通了端到端证据 ✔：给宿主的桥映射表加一条电源键后，**正常启动**生成的 `voxdeck-shortcuts.json` 里，**5 个 Profile 都出现了**：

```json
{"name":"power","label":"电源键","vk":"0xFF","scan":"0x5E","enabled":false,"suppress":false,"mode":"passthrough","shortcut":"none","sourceType":"keyboard"}
```

`enabled/suppress = false` + `passthrough` ✔ = **我刻意选的"未配置就不拦截"默认** ✔：**在你给它指派动作之前，Windows 对电源键的处理完全不变** ✔（这条默认行为是被**产物**证实的 ✔，不是猜的 ✔）。

### 二、为什么我**没有**把这些改动留下（✗ 我把它弄红了）

同一条改动牵动了**四处编码了"不做电源键"这个旧决策的地方** ✗，我这一轮只改了两处 ✗：

| 位置 | 旧决策的表现 ✗ |
| --- | --- |
| 宿主自测 | **断言 `power` 映射必须不存在** ✗（我改了它 → 通过 ✔） |
| 界面 `GestureLayerKeys` | 加了 power 行 ✔，但**配套表没同步** ✗ → 自测报「The gesture layer table does not match the configurable physical keys」✗ |
| 桥的默认表 + fixture | 我加了默认表 ✔，但 fixture 的删改**把编译弄坏了** ✗ |
| 文档 | `QUICK_START_ZH.md` 仍写着「开机键**没有稳定 Windows 事件**」✗ |

### 三、我这一轮的**两次失误**（都撤回，不留"看起来绿"的假门禁 ✗）

1. **在桥自测里加了一条断言，但它根本不会被执行** ✗ —— 我用**负对照**证明了这一点 ✔：把 vk 故意写成非法的 `0x100`，自测**依然 passed** ✗。既然它看不见失败 ✔，它就不是门禁 ✗ → **撤掉** ✔。
2. **按行删除 fixture 条目时删多了** ✗ → 留下一处多余括号 ✗ → 桥**编译失败** ✗；同时发现宿主自测因界面配套表未同步而失败 ✗。

**处置**：`git checkout --` 把两个产品文件**整体回退** ✔ → 重新确认 **host self-test ✔ / bridge self-test ✔ / validate ✔ / 工作区干净 ✔**（当前状态就是绿色 ✔）。**结论：宁可回到干净状态，也不留一个半成品和两条打不中的测试** ✔。

### 四、下一次的正确打法（范围已精确 ✔）

**一次性改齐六处** ✔，再验证 ✔：① 宿主映射 ✔ ② 界面 `GestureLayerKeys` **与它的配套表** ✔ ③ 桥默认表（**别动 fixture 的行结构** ✔，或整条替换 ✔）④ 三处自测期望（**反转**为"存在且值正确" ✔，而不是删除 ✗）⑤ `QUICK_START` 的事实更正 ✔ ⑥ 门禁 ✔。
**验证顺序**：host self-test ✔ → bridge self-test ✔ → 生成的 `voxdeck-shortcuts.json` 里含 `vk:0xFF/scan:0x5E`（5 个 Profile ✔）→ `validate` ✔ → 再请你**按一次电源键** ✔（这一步只有你能做 ✗）。
**关于第 4 步"用起来"**：映射一旦被指派动作 ✔，`ConfiguredMapping` 会把该键变成**拦截 + suppress** ✔（其它键走的正是同一段代码 ✔）—— 这一步我**尚未用产物验证过** ✗（本轮没走到 ✔），所以不当作已验证 ✔。

## 2026-09-12 电源键支持**已落地**（六处一次改齐 ✔，含三处"编码了旧决策"的测试）

按上一轮定下的六处计划一次改齐 ✔，并且这次**先读出了所有编码旧决策的地方**再动手 ✔：

| # | 位置 | 改动 |
| --- | --- | --- |
| ① | `VibeMic.BuildBridgeMappings` | 新增 `power` 映射：`vk=0xFF` / `scan=0x5E` ✔，动作取用户「电源键」条目，**默认 none** ✔ |
| ② | 界面 `GestureLayerKeys` | 新增 `{ "power", "电源键", "" }` ✔（单层 ✔，长/双击行读作未配置 ✔） |
| ③ | 界面表自测 | 数量 **8 → 9** ✔、**必须有 power** ✔、**仍必须没有 voice** ✔、`GestureConfigKey("power",…)` ✔ |
| ④ | 宿主自测的 power 期望 | 由「**必须不存在**」✗ **反转为**「存在且 `vk=0xFF`/`scan=0x5E`/未启用/未拦截/`passthrough`」✔ |
| ⑤ | 桥的默认表 | 新增同一条 ✔（宿主文档缺失时的兜底 ✔） |
| ⑥ | 文档 | `QUICK_START_ZH.md` / `FEATURES_ZH.md` / `GITHUB_RELEASE_BODY_ZH.md` 更正为事实 ✔（`V1_2/V1_3` 历史验收记录**不改** ✔） |

**门禁**：那条"页面不得出现电源键"的旧门禁**反转为**"必须有电源键、仍不得有返回/音量键" ✔；文档门禁改为**两种措辞都接受** + **面向当前的文档必须提到电源键** ✔；另加一条门禁把六处钉在一起 ✔。

### 验证（三层 ✔）

1. **host self-test ✔**（新增的断言**在跑**并通过 ✔ —— 与上一轮那条"打不中"的断言不同 ✗）
2. **bridge self-test ✔**
3. **产物 ✔**：正常启动生成的 `voxdeck-shortcuts.json` 里，**5 个 Profile 全部**含
   ```json
   {"name":"power","label":"电源键","vk":"0xFF","scan":"0x5E","enabled":false,"suppress":false,"mode":"passthrough"}
   ```
   `enabled/suppress=false + passthrough` ✔ = **在你给它指派动作之前，Windows 对电源键的处理完全不变** ✔

### 我这一轮的两个失误（都已修正 ✔，值得记 ✗）

1. **脚本化编辑又产生部分插入** ✗（计数/检查进去了，界面行与桥映射没进去 ✗），并且**丢掉了 `voice` 的负向检查与 `if` 的收尾括号** ✗ → 宿主编译报 `CS1525` ✗。教训：这种跨多处的改动**只用 edit 工具逐处改** ✔（本轮后半程就是这样 ✔）。
2. **把桥文件里的字符串放进了 `includesAll(app, …)`** ✗ —— 和上一轮同一类错误 ✗（app 指 `VibeMic.cs` ✗）→ 门禁失败 ✔。教训：**每条字符串都要问"它住在哪个文件"** ✔。

### 仍未验证（如实 ✗）

- **实际按键**：需要你按一次电源键 ✔（只有你能做 ✗）。目前可确认的是：映射已生成 ✔、键位识别路径（`VK 0xFF / 0x5E`）在代码里存在且掩码保留了 `0x5E` ✔。
- **"指派动作后变成拦截+suppress"**这半条：走的是与其它键**同一段代码** ✔，宿主自测目前只断言了**未配置**时的状态 ✔ —— 这一半**没有**被产物或自测单独验证过 ✗，我不当作已验证 ✔。

## 2026-09-12 电源键**界面卡片**补齐（上轮我漏了这一步 ✗，本轮的检查把它抓了出来 ✔）

### 我上轮的漏洞 ✗

上轮我把电源键加进了 `GestureLayerKeys` 表 ✔ 就以为"界面入口"完成了 ✗ —— **错** ✗。这轮我先**看界面**（而不是再看代码 ✔），dump 里**完全没有「电源键」** ✗，追下去发现：**快捷键页的卡片是显式调用建出来的** ✗（`AddMappingOverviewCard(canvas, previewRemote, new Point(…), "上键", "上键", "up", …)` ✗），那张 `GestureLayerKeys` 表**不驱动卡片** ✗ → 所以上轮的电源键**只在配置里可映射，用户却没有入口去指派** ✗。

### 本轮补的两处

1. **左列第 4 行新增电源键卡片** ✔（与右列的 TV 同一行 ✔，画布内 ✔）：
   `AddMappingOverviewCard(… new Point(18, 78 + GestureCardPitch * 4), "电源键", "电源键", "power", "电源键", "", false)` ✔
2. **应用内那条旧声明也改了** ✗→✔：页面底部原来写着「**开机、返回和独立音量键在 Windows 下无稳定事件，不提供映射**」✗ —— 我上轮只改了**文档**，**没改应用内文案** ✗。
   新文案 ✔：「**电源键可配置**；返回与独立音量键在 Windows 下无稳定事件，不提供映射。…」

### 验证（看界面，不是看代码 ✔）

- **文字判据**：页面里出现 `[电源键]` ✔ 与新文案 ✔
- **目视**（加高窗口 1280×1500 让整页可见 ✔）：标题「**电源键**」+ 徽章「**可配置**」✔ + **短/长/双** 三行**均为「未配置」** ✔（可点 ▶ 测试 ✔），下方新文案 ✔
- **几何**：`no overlapping sibling controls and no clipped text` ✔
- `validate` ✔、host/bridge self-test ✔
...
## 2026-09-12 **目标第 3 步（菜单/返回/音量键）判断完成** ✔：菜单已支持，返回与音量**在用户态拿不到** ✗，不做 ✔

### 外部实测证据（MiVibe-Remote 的硬件记录，非推断 ✔）

`docs/HARDWARE_MAP.md`（实机采样 ✔，设备 `VID 0x2717 / PID 0x32B8` ✔）关键行：

| 按键 | Windows 事件 | 状态 |
| --- | --- | --- |
| **开关（电源）** | `VK 0xFF` / `E0 5E` | **已确认** ✔，并注明「**轻触无系统动作**」✔ |
| 上/下/左/右/确认 | 标准键事件 | 已确认 ✔ |
| Home / 菜单 / TV | `VK_HOME 0x47` / `VK_APPS 0x5D` / `VK_OEM_3 0x29` | 高置信 ✔ |
| 麦克风 | `VK_F5 0x74` / `0x3F` | 高置信 ✔（约 32ms 自动重复 ✔） |
| **返回 / 音量 + / 音量 −** | **无 Raw Input、无翻译键事件、无 `WM_APPCOMMAND`** ✗ | **不进入 Windows 普通输入栈** ✗ |

它还写明：共享只读打开该键盘 collection 的 HID input report 会得到 **`Access Denied (5)`** ✗ → 用户态**只能读 descriptor 能力，读不到那 121 字节原始 report** ✗；要用这些键**必须有 HID 层旁路**（它用 WUDFHost/Frida 限时探针读到返回 `F1`、音量 `80/81` ✗），而它的 0.3 把这条做成**独立管理员辅助进程 + 每次启动手动启用 + 三键校准** ✗，且其发布说明仍写着**持续使用/重连验收未完成** ✗。

### 判断（成本 vs 收益 ✔）

- **菜单键**：**我们已经支持** ✔ —— 我们的「功能键」映射正是 `Apps / 0x5D` ✔，与它的一致 ✔ → **无需改动** ✔
- **返回 / 音量 ±**：**用户态拿不到** ✗（两条独立采样一致 ✔），要做就得引入 **HID 层管理员辅助进程 + 每次启动启用 + 三键校准** ✗，而收益只是"返回=Backspace、音量=切相邻任务" ✗ —— **这两个功能用我们已有的可配置键（上/下/左/右/确认/Home/TV/功能键）就能实现** ✔ → **判断：不做** ✔（并说明这不是我们偷懒 ✗，是 Windows 输入栈层面的限制 ✔）
- **电源键**：**可达** ✔（我们已实现 ✔），**且不需要系统级映射** ✔ —— 它的实测写明「轻触**无系统动作**」✔，而我们的低级钩子能直接看到该键 ✔ → MiVibe 那套**系统级扫描码重映射**留作**按需后备** ✗（只有当你机器上 Windows 会抢先处理它时才需要 ✔，默认关闭 ✔）
...
## 2026-09-12 我发现**自己的修复会引入回归** ✗ 并修掉了它 ✔：未指派的电源键必须让位给录音兜底

### 风险（读本机日志推出来的 ✔）

上一条日志（02:14:33「Voice raw VK fallback vk=0xFF scan=0x5E」✔）说明：**RC003 重连后会把麦克风键上报成 `0xFF/0x5E`** ✗ —— 也就是这个码**本身是歧义的** ✗：既可能是电源键（MiVibe 硬件表 ✔），也可能是重连后的录音键（我们的兜底逻辑 ✔）。

而我上一轮的规则是"**只要 power 映射存在就压过兜底**"✗ → 后果是：**未指派的电源键会把 `0xFF/0x5E` 从录音兜底手里夺走** ✗ → **重连后按住麦克风键就不能录音了** ✗✗ —— 这是我引入的回归 ✔，而它只有在用户真实使用中才会暴露 ✗。

### 正确的规则（本轮落地 ✔）

把两处调用（`VoxDeckInputBridge.cs` 601 ✔ 与 3229 ✔）从

```csharp
if (mapping == null && IsVoiceRawCandidate(...))
```
改为

```csharp
if ((mapping == null || !mapping.enabled) && IsVoiceRawCandidate(...))
```

含义 ✔：
- **未指派**（`power` 映射 `enabled=false` ✔）⇒ **不阻挡兜底** ⇒ `0xFF/0x5E` 仍按录音键处理 ✔ —— **今天的麦克风行为完全不变** ✔
- **已指派**（`enabled=true` ✔）⇒ 映射胜出 ⇒ **执行用户动作并吞掉该键** ✔，兜底不生效 ✔
- 对其它键**无影响** ✔（`IsVoiceRawCandidate` 只认 `F5 / 0xF5 / 0xFF+0x5E` 三种形态 ✔，其它键不在其中 ✔）

### 验证 ✔

- 两处落地并**逐条确认行号与文本**（601 / 3229 ✔）
- **宿主 self-test ✔ + 桥 self-test ✔**（桥的协议/映射自测仍过 ✔）
- **门禁已同步**为新的规则文本 ✔（旧的形态门禁按预期失败 ✗→已更新 ✔），并写明"**未指派必须让位**"的理由 ✔
- `validate` ✔
...
## 2026-09-12 快捷键页**按实物重排**（十字形方向环 ✔）—— 用户选定方向后实施

### 用户的选择（按技能规程先给方向 ✔）

- **布局方向**：**十字形方向环**（最贴实物 ✔）
- **返回键 / 音量摇杆**：**不显示** ✗，只在底部文案说明 ✔

### 实物布局的权威来源（应用自己的插图 ✔）

`RemoteVisual.OnPaint`（`VibeMic.cs` 26320-26346 ✔）就是应用对实物的建模 ✔：

| 设计坐标 | 控件 |
| --- | --- |
| (29,29) ∥ (83,29) | **开机键 ∥ 录音键**（并排最上 ✔） |
| (56,96) r=43 | **方向环**（上/下/左/右 + 中心＝确认 ✔） |
| (29,158) / (83,141-219) | 返回键 / 音量摇杆（实物有 ✔，用户态不可达 ✗） |
| (29,199) | Home ✔ |
| (29,240) ∥ (83,240) | 功能键 ∥ TV ✔ |

### 改后的布局（三列栅格 18 / 337 / 656，卡 286×152，行距 164 ✔）

```
行1  电源键(18)       录音键(337, 固定卡)
行2                   上键(337)
行3  左键(18)         确认键(337)      右键(656)
行4                   下键(337)
行5  Home(18)         功能键(337)      TV(656)
行6+ 遥控器插图(18,950) + 手势分层图例(656,950) + 底部说明(18,1450)
```

- 中列 **337 = 实物中心线** ✔（`RemoteVisual` 里方向环中心是 `sx(56)` ✔）
- 画布由 **960×960 → 960×1520** ✔（插图与图例移到栅格下方 ✔），滚动高度相应 +560 ✔
- 每张卡的**接线未变** ✔（`physicalKey / label / remoteControl / shortKey / longKey` 逐槽核对过 ✔）

### 验证（技能核对表 ✔）

- `validate` ✔；host self-test ✔
- **几何**：`1280×1800` 强制尺寸下 **0 重叠 / 0 裁切** ✔（`no overlapping sibling controls and no clipped text` ✔）
- **目视**：新布局截图（顶行电源+录音并排 ✔、十字方向环 ✔、功能键行 ✔、插图在最上方按钮与卡片顶行对应 ✔）
- 六个页面仍可达 ✔（几何检查遍历六页 ✔）、录音键仍为「固定稳定链路」不可配置 ✔、主题/自检/隐私文案未触及 ✔
- 发布链 ✔（12/12 界面矩阵 ✔）、安装器 exit 0、**51/51 逐文件一致** ✔、冻结采集件哈希未变 ✔
- 门禁新增一条 ✔：钉住整套栅格坐标与"插图/图例已下移、画布已加高" ✔
...
1. **截图工具抓到的是安装版的旧构建** ✗ —— 我杀进程时用的是 `VibeMic` ✗，而安装版进程名是 **`VibeFlow.exe`** ✗ → 抓到的是**旧布局** ✗。**发现方式**：我在换图**之前先看了新图** ✔（否则会把一张旧布局的图加进文档 ✗）。处置：`git checkout` 回退图 ✔ → 先跑发布链与安装 ✔ → 再重拍并**再次目视确认** ✔。
2. **`.NET` 静态调用不认 PowerShell 的当前目录** ✗（`ReadAllText('docs\...')` 解析到了会话目录 ✗，报文件不存在 ✔）→ 改用**绝对路径** ✔。（cmdlet 的相对路径正常 ✔，所以图片复制是成功的 ✔。）

## 2026-09-12 快捷键页**再优化**：从"十字形"改为**规整两列宽卡**（用户反馈十字布局"太丑" ✗）

### 用户的反馈与要求

1. **最上面的两个键（开机 / 录音）要分列左右** ✗→✔ —— 上一版把录音键放在中列 ✗，不符合"左开机、右录音" ✔
2. **其余按键要协调美观** ✗→✔ —— 上一版十字布局有两行只放一张卡，**左右各留一大片空白** ✗，视觉上是碎的 ✗
...
- 卡宽 **286 → 448** ✔，栏间距 **28** ✔ → 两列 x = **18 / 494**（右缘仍是 942 ✔，与页面既有边界一致 ✔）
- **卡片内部改为按自身宽度排布** ✔：徽章 `GestureCardWidth - 144` ✔、动作框宽 `GestureCardWidth - 106` ✔、▶ 在 `GestureCardWidth - 36` ✔ —— 否则加宽后徽章与行会全挤在左边 ✗
- 行距仍 **164** ✔，栅格底 **886** ✔（插图 950 / 图例 656,950 / 说明 18,1450 / 画布 960×1520 均不变 ✔）

### 验证

- 几何：`1280×1800` 下 **0 重叠 / 0 裁切** ✔
- 目视：两列对齐 ✔、每行满 ✔、顶行"电源键 ∥ 录音键" ✔、动作框与 ▶ 在各卡内对齐 ✔
- host self-test ✔、`validate` ✔
- 门禁同步为新的栅格与卡宽契约 ✔（旧的三列坐标已失效 ✗→已更新 ✔）

### 收尾：文档图与基线锁（本轮第二次改布局后 ✔）
...
用户要求：「把遥控器的图形和对应的显示，放在左右排快捷键的中间」✔

### 布局（三列：卡片 ∥ 遥控器 + 其说明显示 ∥ 卡片 ✔）

```
   左列                中间                      右列
   电源键（开机）      小米蓝牙遥控器 2 Pro       录音键（固定稳定链路）
   左键                按下实体键时…会高亮 ✔      右键
   上键                遥控器图形                下键
   确认键              手势分层图例 ✔             Home 键
   功能键                                        TV 键
...
- 三列宽度正好合上页面原有边界 ✔：`18 + 286 + 26 + 300 + 26 = 656` ✔ → 右列 656..942 ✔
- 遥控器图形的**「对应的显示」**= 它上方的标题与说明（`canvasTitle`/`canvasState` ✔，本来就居中于中列 ✔）+ 下方的**手势分层图例** ✔（因此图例从中列下方回到图形正下方 ✔）
...
- **目视**：图形在正中被两列卡片夹住 ✔；左右两列**顶格分别是电源键 / 录音键** ✔（用户此前的第 1 点仍满足 ✔）；图形下方即为手势分层 ✔
...
## 2026-09-12 文案优化第一批（用户点名的两处 ✔ + 同族清理 ✔）

### 先清点，再改（不凭印象 ✗）

用 `vibe-page-shot.ps1 -DumpText` 把**六个页面**的真实文案 dump 成文件（`home 63 / workflow 26 / shortcut 75 / voice 37 / selfcheck 71 / settings 62` 行 ✔），逐条判"简明清晰" ✔。

### ① 快捷键页的「回退」——用户点名 ✗→✔

| 旧 ✗ | 新 ✔ |
| --- | --- |
| 卡片：`未配置（回退到短按：显示桌面）` | `未设置（跟随短按：显示桌面）` |
| 按钮：`设为回退` | `设为默认` |
| 标签：`回退 Profile` | `默认 Profile` |
| 说明：`无匹配时使用回退方案` | `没有匹配的应用时用默认方案` |
| 图例：`某一层没有单独配置时回退到上一层，卡片会写明回退目标` | `某一层没单独设置时，就按上一层执行；卡片上会写按哪一层` |
| 确认文案：`按键服务已确认回退 Profile：…` | `已设为默认 Profile：…` |
| 工作流步骤：`应用关联 Profile（智能切换开启时设置回退）` | `…（智能切换开启时设为默认）` |

涉及常量 `GestureBindingStore.UnboundLayerText`（未配置→**未设置** ✔）、`FallbackPrefix`（回退到→**跟随** ✔），以及 `VibeMic.cs` 的 5433 / 7654 / 7672 / 7753 / 8099 / 8121 / 8128 / 8129 / 9091 ✔ 与 `ProjectSpaceRunner.cs:623` ✔。
**验证**：`grep 回退` 在源码中**只剩一条解释性注释** ✔；页面文字判据实测 `未设置（跟随短按：显示桌面）` ✔ 与新的图例 ✔。

### ② 首页文案（用户点名"太多" ✗）——先去掉噪音 ✔

| 旧 ✗ | 新 ✔ |
| --- | --- |
| 副标题 `语音桥接、快捷键和设备状态` | `语音、按键与设备状态` |
| 状态词 `VOICE LINK OFF` / `TRIGGER ONLY` / `PUSH TO TALK` / `PUSH TO TALK READY` / `CONNECTING` ✗（zh-CN 界面里的英文 ✗） | `语音未启动` / `仅触发模式` / `按住说话` / `已就绪` / `连接中` ✔ |

**验证**：页面文字判据显示副标题与「语音未启动」✔，且**全页不再出现这些英文状态词** ✔（脚本检查 ✔）。
（首页剩下的"六段连发文案 / 步骤条"等 ✗ 留到下一批 ✔ —— 本轮先把噪音最大的英文与冗长副标题清掉 ✔。）

### 一次失误（第 6 次同类 ✗，但这次**自测抓住了** ✔）

我改了「设置回退 Profile」这一族的**自测期望** ✗ 却漏改**生产代码**的 3 处 ✗ → `host self-test` 立刻报 `Project Profile claimed the Smart Profile was currently effective` ✔ → 补齐后通过 ✔。**教训**：字符串是**跨文件语义耦合**的 ✔ —— 改名要**一次找齐所有出现点**（本轮 `grep 回退` 全量核对 ✔），而不是按记忆改 ✗。

### 验证汇总

`grep 回退` 全量核对 ✔、两个页面文字判据 ✔、host self-test ✔、`validate` ✔、几何 `1280×1400` **0 重叠/0 裁切** ✔。

### 收尾（文案第一批 ✔）

- 从**安装版新构建**重拍 `docs/images/03-shortcuts.png` 与 `03-shortcuts-screenshot.png` ✔（避免文档里还留着「回退」的旧截图 ✗）
- 基线锁该行哈希已更新 ✔（→ `3125477C…`）
- **证据说明** ✔：新文案「未设置（跟随短按：显示桌面）」位于第 4 行（Home 卡片），**不在 1280×840 的可视区内** ✗ → 因此该项的依据是**页面文字判据**（实测通过 ✔），而不是截图 ✔ —— 截图只用于避免文档图片过期 ✔
- 发布链 ✔（12/12 矩阵 ✔）、安装器 exit 0、**51/51 一致** ✔、冻结采集件哈希未变 ✔
- 本轮失误记录 ✔：改「回退」时**漏改生产代码 3 处** ✗（自测抓住 ✔）、以及**漏改第三处英文状态词** ✗（新增门禁抓住 ✔）→ 教训：**改名要一次找齐所有出现点**（grep 全量核对 ✔）

## 2026-09-12 文案优化第二批：**首页**（用户点名"文案太多了" ✗）

### 改了什么（7 处 ✔）

| 旧 ✗ | 新 ✔ | 理由 |
| --- | --- | --- |
| `按住说话 · 松开结束 · 用户确认发送` | **`不自动发送 · 文字由你确认`** | 原句把内部流程当文案 ✗；新句用大白话承载**隐私声明**（技能硬约束：这一条必须始终可见 ✔） |
| 步骤 `按住录音键 / 持续说出内容 / 松开结束录音` | `按住录音键 / 说出内容 / 松开结束` | 每步少一个词，三格宽度下更清爽 ✔ |
| `已就绪，等待按住录音键` | `等待按住录音键` | 「已就绪」是冗余前缀 ✗ |
| 常用按键行 `短 显示桌面 / 长 未设置` | **`短按 显示桌面 · 长按 未设置`** | 单个「短/长」需要用户猜 ✗ |
| `等待一次真实按键操作`（两处） | `等待按键操作` | 「一次真实」是内部口径 ✗ |
| `通用导航 · 按下已配置按键后显示真实执行结果` | `通用导航 · 按下已配置按键后，这里显示真实结果` | 补上"这里"，句子完整且更短 ✔ |

### 验证

- **页面文字判据** ✔：实测出现 `不自动发送 · 文字由你确认` / `说出内容` / `等待按住录音键` / **`短按 显示桌面 · 长按 未设置`** / `等待按键操作` / `…这里显示真实结果` ✔
- **几何** ✔：常用按键行**变长**了 ✗（有撞徽章的历史 ✗），因此专门查了碰撞 → `01-home: overlaps=0 clipped=0` ✔，整页 `no overlapping sibling controls and no clipped text` ✔
- host self-test ✔、`validate` ✔、门禁已钉住这几句与隐私声明 ✔
- 页面上的**三段重复**（副标题 / 提示 / 三步骤卡）现在各司其职 ✔：副标题说状态、提示只说隐私、卡片说怎么做 ✔

## 2026-09-12 文案优化第二批补充：首页「常用按键」摘要（含一次**静默截断**的修复 ✔ 与一个**工具盲区** ✗）

### 我引入又修掉的问题 ✗→✔

把摘要从 `短 显示桌面 / 长 未设置` 改成更清楚的 `短按 显示桌面 · 长按 未设置` 后 ✗，**WinForms 静默省略了后半句** ✗（`AutoEllipsis` ✔），界面上只剩 `短按 显示桌面 · 长…` ✗。

**关键教训（工具盲区 ✗）**：`check-ui-geometry.ps1` 测的是**控件边界**，**测不出文字被省略号吃掉** ✗ → 它当时报的是 `overlaps=0 clipped=0` ✔ —— **干净结果 ≠ 没问题** ✗。这一条只能靠**看图**发现 ✗（我正是看图发现的 ✔）。

### 修法（两条一起用 ✔）

1. **让摘要更短** ✔（也正好更符合"简明"的要求 ✔）：`GestureMappingSummary` 在**长按没有独立动作**时**不再显示**「长按 未设置」✗ → 该行只写 `短按 显示桌面` ✔
2. **把格子修对** ✔：动作列 `128 → 150 px` ✔、字号 `9 → 8.4 pt` ✔（仍在 8 pt 底线之上 ✔）、按键名列 `68 → 52 px` ✔ 并左移动作列 ✔

### 过程中的一个**自伤** ✗（第 7 次同类，已改做法 ✔）

我用**脚本化的多行替换**加注释时 ✗，把 `value.Size = new Size(128, 24);` **整行删掉了** ✗ → 标签退回默认宽度 → 截断更严重 ✗。**发现方式**：grep `value.Size` 全量核对 ✗ → 发现该行根本不存在 ✔ → 用 **edit 工具**补回 ✔。
**做法固定**：**多行改动只用 edit 工具，脚本只做单行替换** ✔（这条我已经写过一次 ✗ —— 这次是它第二次咬我 ✔，所以本轮把它写进门禁注释与文档两处 ✔）。

### 验证

- **目视**（这次是唯一能验证的方式 ✔）：`录音 · 按住听写 / 松开结束` ✔、`Home · 短按 显示桌面` ✔、`功能键 · 短按 复制 · 长按 粘贴` ✔ —— 三行**均完整无省略** ✔
- host self-test ✔、`validate` ✔、几何 `1280×840` 0 重叠/0 裁切 ✔
- 门禁钉住：动作列宽度 ✔、字号 ✔、"长按未设置时不显示"这条规则 ✔

## 2026-09-12 首页摘要：一次自伤、被**界面矩阵**抓住、并按**实测**纠正 ✔

### 事故链（全部记下 ✗）

1. 我把摘要写成更清楚的 `短按 显示桌面 · 长按 未设置` ✗ → 该串在 128px 格子里**被省略号吃掉后半句** ✗（`AutoEllipsis` ✔）—— **几何检查测不到** ✗（它测控件边界 ✔，当时报 0 重叠/0 裁切 ✔）
2. 我用**脚本化多行替换**加注释 ✗ → 把 `value.Size` **整行删掉** ✗ → 标签退回默认宽度 → 截断更严重 ✗
3. 我据此**加宽格子到 150px 并缩小字号** ✗ → 150px 从 x=24 起算到 **252** ✗，而第二列圆点在 **248** ✗ → **4px 重叠** ✗ → **发布链的界面矩阵当场失败** ✗（`interface matrix failed`，3 例默认尺寸 ✗）

### 纠正（按实测，不按推断 ✔）

用 `Graphics.MeasureString` 量了真实字宽 ✔：

| 串 | 9pt | 8.4pt |
| --- | --- | --- |
| `按住听写 / 松开结束` | 116px | 108px |
| `短按 显示桌面` | 82px | 77px |
| `短按 复制 · 长按 粘贴` | 121px | **113px** |

→ 结论 ✔：摘要在**丢掉了「长按 未设置」这半句之后**（那是更简明的一步 ✔），最长也只有 **113px** ✔ —— **原来的 128px 格子本来就够** ✔，**我那次加宽既是多余、又正是重叠的原因** ✗。
→ 于是**把几何恢复原状** ✔（按键列 68 ✔、动作列 x+94 ✔、宽度 128 ✔），只保留 **8.4pt** 以留出余量 ✔（仍在 8pt 底线之上 ✔）。

### 验证

- **界面矩阵 12/12 通过** ✔（三种主题 × 四种尺寸 ✔，默认尺寸的重叠已消失 ✔）
- host self-test ✔、`validate` ✔
- 目视 ✔：`录音 · 按住听写 / 松开结束` ✔、`Home · 短按 显示桌面` ✔、`功能键 · 短按 复制 · 长按 粘贴` ✔ 三行完整无省略 ✔

### 三条教训（写进门禁注释与本文档 ✔）

1. **几何检查看不到文字被省略** ✗ —— 它测边界 ✔。这类问题**只能靠看图** ✔（或量字宽 ✔）。**干净结果 ≠ 没问题** ✗。
2. **多行改动只用 edit 工具** ✔ —— 本轮它**第二次**咬我（静默删行 ✗）。脚本只做**单行**替换 ✔。
3. **先量再改** ✔ —— 我"觉得放不下"就加宽 ✗，实测才发现**本来放得下** ✔。宽度/字号这类判断应当**测量**，而不是估算 ✗。

## 2026-09-12 文案优化第三批：**自检页**（六页里文案最多的一页 ✗）

### 先清点 ✔

`-DumpText` 实测 **71 行** ✔（六页里最多 ✔）→ 问题集中在**每行重复的标签**与**几条长术语句** ✗。

### 改了什么（11 处 ✔）

| 旧 ✗ | 新 ✔ | 理由 |
| --- | --- | --- |
| 每行都以 **`当前状态：`** 开头（×10 ✗） | **去掉该前缀** ✔ | 行标题已经框定了这句话，重复十次纯噪音 ✗ |
| 分组前缀 `核心环境 · …` / `语音链路 · …` / `V2.0 场景能力 · …` | **`核心 · …` / `语音 · …` / `场景 · …`** ✔ | 前缀每行重复，收短即可 ✔ |
| `检测到重复进程、其他安装目录进程或无法确认来源的进程` | **`有重复或来源不明的进程`** ✔ | 20 字 → 11 字 ✔ |
| `CABLE Output 当前被 Windows 标记为线路设备，只列出麦克风的语音工具看不到它` | **`CABLE Output 被当作线路设备，部分语音工具看不到它`** ✔ | 该句原来**占两行** ✗ |
| `稳定语音档案 v11 已应用` | **`语音参数已应用（v11）`** ✔ | 模型版本号不必占句首 ✗ |
| 输入法行尾 `· 言灵所在输入法上下文：微信输入法（与配置一致）` | 去掉该尾句 ✔（详情里仍有 ✔） | 状态行只说状态 ✔ |
| `ctrl + win`（小写 ✗） | **`Ctrl + Win`** ✔ | 改用应用**既有**的 `MappingShortcutDisplay` ✔（顺带统一了另外两处 ✔） |
| 副标题 `每项给出结论与修复入口；点「详情」看…` | `点「详情」看正确状态、原因与下一步` ✔ | 前半句是重复 ✗ |
| `从第一项错误开始修复；返回言灵后会自动复检，…` | `从第一项开始修复；返回言灵会自动复检，…` ✔ | 去掉冗余的"错误""后" ✔ |

### 验证

- **页面文字判据** ✔：行数 71 → **70** ✔；`当前状态：` **零残留** ✔；实测出现 `CABLE Output 被当作线路设备，部分语音工具看不到它` ✔、`语音参数已应用（v11）` ✔、`微信输入法 已就绪 · Ctrl + Win · 单击切换（与配置一致）` ✔
- host self-test ✔、`validate` ✔、门禁已钉住这批文案与"分组名收短""热键用统一格式化器" ✔
- 一处拼接确认 ✔：我改掉输入法长尾后，尾部 `（与配置一致）` 仍正常显示 ✔（没有把表达式改坏 ✔）

## 2026-09-12 文案优化第四批：**设置页**（62 行 ✔，但真正的问题只有**三段长句** + 一个术语 ✗）

### 清点结论 ✔

设置页的**术语表**六条（`快捷键 Profile` / `Smart Profiles` / `Browser Remote Lite` / `Raw Input` / `Live HUD` / `Context Deck` ✔）**每条都已有白话解释** ✔ —— 那是更早一轮补的 ✔，所以本轮**不需要再动术语本身** ✔（术语要保留，文档与自检页都在用它们 ✔）。

真正要改的是**行文** ✗：

| 旧 ✗ | 新 ✔ |
| --- | --- |
| 签名通道那条**三句**长文：`未安装签名通道时，言灵不会拦截来源未知的键，因此实体键盘保持原行为；自定义遥控器键的原始按键效果可能同时发生。签名通道是可选增强，不影响动作执行。` | **两句** ✔：`未安装签名通道时，言灵不拦截来源未知的键：实体键盘保持原样，遥控器自定义键的原始效果可能同时发生。签名通道是可选增强。`（三个事实一个不少 ✔：键盘不受影响 / 原始效果仍可能发生 / 通道是可选的 ✔） |
| `普通日志只记录连接状态与聚合指标，单个日志自动限制为 4 MB。诊断音频必须每次明确确认。` | `普通日志只记录连接状态与聚合指标，每个日志上限 4 MB。诊断音频每次都要你明确确认。` ✔ |
| `稳定语音档案 v11  ·  配置 schema 32` | **`稳定语音档案 v11  ·  参数版本 32`** ✔（`schema` 是代码的词 ✗，不是用户读的词 ✔） |
| `统计范围是当前本地日志窗口（日志限长，旧记录会随滚动丢弃），且只统计有结束回执的会话；不包含录音、转写文字、窗口标题或设备地址。` | `只统计当前日志窗口内有结束回执的会话（日志限长，旧记录随滚动丢弃）；不含录音、转写文字、窗口标题或设备地址。` ✔ |

### 保留未动（有意 ✔）

- **术语表本身** ✔ —— 术语 + 白话并列是最清楚的形式 ✔
- **隐私三句**（`本地安全模式：默认不保存录音、不上传音频、不读取听写文字` ✔ / 日志范围 ✔ / 统计不含内容 ✔）—— 技能硬约束要求这些**必须可见** ✔，只压缩措辞、不删事实 ✔
- 按键来源保护那句（`尚未逐设备隔离：…（签名通道为可选增强）`✔）—— 它是 P0-2 修复后的标准表述 ✔，含义完整 ✔

### 验证

`validate` ✔、host self-test ✔、页面文字判据实测四条新句全部出现 ✔（含 `参数版本 32` ✔）。

## 2026-09-12 文案优化第五批：**语音页 + 工作流页**（含一个**双句号 bug** ✗→✔）

### 语音页（37 行 ✔）

| 旧 ✗ | 新 ✔ |
| --- | --- |
| **`…也不会自行上传音频。。另外：…`** ✗✗ | **双句号消失** ✔ —— 根因：`VoiceProviderCompatibilityNote` 返回的串**以「。」开头** ✗，而它被拼在一句已结束的话后面 ✗（真 bug ✔，用户直接看得到 ✗） |
| `注：微信输入法语音会把文字放进剪贴板；…请目视确认后再按确认键发送。识别建议：在微信输入法语音设置中选择普通话、保持 10–20 厘米说话距离并减少背景噪声，可显著降低错字率。` | `另外：微信输入法会把文字放进剪贴板，言灵在录音结束后自动粘贴到已聚焦的输入框（不读取文字），请目视确认后再发送。识别建议：选普通话、保持 10–20 厘米距离、减少背景噪声。` ✔（建议改成**并列短语** ✔） |
| `● 稳定档案 v11 已应用` | **`● 语音参数已应用（v11）`** ✔（与设置页统一 ✔） |
| `固定为首版稳定模式；松开立即结束，单次最长约 60 秒。` | `首版稳定模式；松开立即结束，单次最长约 60 秒。` ✔ |

**有意不动** ✔：`启动快捷键` 那是**可编辑输入框**（显示 `ctrl+win` ✗）—— 改成 `Ctrl + Win` 会让**保存时写回带空格的格式** ✗，风险大于收益 ✗；它下面那行 `稳定参数：Ctrl + Win · 单击切换` 已经给了友好读法 ✔。

### 工作流页（26 行 ✔）

| 旧 ✗ | 新 ✔ |
| --- | --- |
| 副标题 `把文字固定送进你常用的应用：…，剩下的交给录音键` | `把文字送进你常用的应用：添加、学习、保存，之后交给录音键` ✔ |
| `选择键位 Profile` | **`选择快捷键 Profile`** ✔（与术语表用同一个词 ✔） |
| `还没有应用工作流：在“快捷键 → Smart Profiles”里为一个本机应用选择键位 Profile，再来学习它的工作流` ✗（长句 + 直角引号 ✗） | `还没有应用工作流：在「快捷键 → Smart Profiles」里选一个 Profile，再回来学习。` ✔ |
| 提示 `…点「打开」可以把未运行的应用冷启动并定位到输入框。` | `…点「打开」可把未运行的应用启动并定位到输入框。` ✔（去掉"冷"这个内部说法 ✗） |

### 验证

- **页面文字判据** ✔：语音页出现 `● 语音参数已应用（v11）` ✔、`首版稳定模式；…` ✔、收紧后的说明 ✔，且脚本检查**全页已无「。。」** ✔；工作流页三条新文案全部实测 ✔
- `validate` ✔、host self-test ✔、门禁已钉住这批措辞与"双句号不得回归" ✔

## 2026-09-12 文案优化第六批：**向导五步**（最后一块 ✔）—— 并纠正我自己的一次误判 ✗

### 先确认改的是**生效那一份** ✔

代码里有三份向导 ✗：`ShowSetupWizard()`（13571 ✔）、`ShowSetupWizardElevenStepLegacy`（14501 ✗）、`ShowSetupWizardLegacy`（15185 ✗）。查调用点：**生效的只有 `ShowSetupWizard()`** ✔（被 6 处调用 ✔），另两份**从未被调用** ✗ → 只改生效那份 ✔。

### 我的一次误判 ✗（值得记）

我的提取脚本报出两个"被截断的步骤短名" ✗（`确认设备与` / `选择工具并` ✗）—— **查证后发现它们在注释里** ✗（那行注释正是**解释此前修复过这个截断问题** ✔），而真正的步骤名是完整的 ✔ 且控件用 `AutoSize = true` ✔。
→ **教训**：**提取文案时必须跳过注释** ✗ —— 否则会把解释文字当成用户可见文案 ✔（本轮没造成改动 ✔，因为我在改之前先读了那一行 ✔）。

### 实际改动（5 处 ✔）

| 旧 ✗ | 新 ✔ |
| --- | --- |
| `尚未取得本次真实音频与工具回执，未确认测试结果` | `还没有收到本次的真实音频与工具响应，测试结果未确认` ✔（去掉"回执"这个内部词 ✗） |
| `语音桥接已启动，但尚未收到真实麦克风就绪证据` | `语音桥接已启动，但还没收到遥控器麦克风` ✔（去掉"证据"✗，并说清是哪个麦克风 ✔） |
| `是否启用 Smart Profiles（按应用自动切换键位方案 · 可选，默认关闭）` | `Smart Profiles：按应用自动切换键位方案（可选，默认关闭）` ✔（去掉疑问句形式 ✗ —— 它出现在设置区而非提问场景 ✔） |
| `再点击“我已看到文字”` ✗（弯引号 ✗） | `再点击「我已看到文字」` ✔（与全局引号风格一致 ✔） |
| `当前任务尚未取得所需证据` | `当前任务还需要完成` ✔（"所需证据"是内部口径 ✗） |

### 保留未动（有意 ✔）

- 五步名称与一句话说明 ✔（`配对 RC003，并用一个真实方向键证明 Windows 已收到遥控器事件。` 这类句子已经是"一步一句" ✔）
- 隐私与诚实披露 ✔：`本地传输 · 不保存录音 / 不读取或记录转译文字` ✔、`! 录音键当前为安全直通：前台应用可能同时收到录音键；设备级精确隔离仍在开发中。` ✔、`请先目视确认文字，再点击此按钮；言灵不会读取测试框内容。` ✔ —— 这些是硬约束要求可见的事实 ✔，一字未删 ✔

### 验证

`validate` ✔、host self-test ✔、门禁已钉住这批措辞（含"直引号不得回归"✔）。
...
```
!  还没有收到刚才的方向键 • ONBOARDING-CHECK-REQUIRED
```

**一句行动指引后面钉着支持用错误码** ✗ —— 读起来像崩溃报告 ✗。已改 ✔：**可见行只留那句话** ✔，错误码移到**悬停提示**（`ToolTip` ✔）里 ✔ —— 支持沟通需要时仍拿得到 ✔，界面不再吓人 ✔。门禁已钉住这条 ✔。

## 2026-09-12 **B 方案执行完毕**：缩小漏键窗口 + 把提示文案改准 ✔

### 一、真正的根因（比我上次说的严重 ✗，必须纠正 ✔）

我上次说漏键是"边缘情形"✗ —— **读代码后不成立** ✗：

| 事实 | 值 |
| --- | --- |
| 抑制窗口 `Rc003PresentRecently()` | **5 秒** ✔ |
| 刷新该窗口的 **Raw Input 健康探测**周期 | **30 秒** ✗ |

→ 于是**每 30 秒里有约 25 秒**，言灵认为遥控器"不在场" ✗ → **一次听写会话的"第一下"录音键不被吞** ✗（恰恰是开始录音那一下 ✔），之后连按才被吞 ✔。**日志完全印证** ✔：`RC003 ISOLATION scoped_suppress=true` 只出现在 `repeat_held=true`（自动重复）的那些行上 ✗，从没出现在第一下 ✔。

### 二、改了什么（行为 ✔）

1. **探测周期 30 秒 → 5 秒** ✔（`int healthIntervalMs = Rc003PresenceWindowMs;` ✔）—— 让探测落进**它自己喂的那个窗口**里 ✔，遥控器在场时该窗口**持续新鲜** ✔ → **第一下也会被吞** ✔
2. **窗口具名** ✔：`private const int Rc003PresenceWindowMs = 5000;` ✔（把"探测必须落在窗口内"这个关系写进代码 ✔）
3. **日志按 30 秒限流** ✔（探测变密后不让日志刷屏 ✔），并**在启动时记一行** ✔：`Raw Input health timer interval_ms=5000 presence_window_ms=5000` ✔ —— 任何日志里都能看出是谁在喂这个抑制 ✔

### 三、文案改准（3 处 ✔，其中一处原本**说的是错的** ✗）

| 旧 ✗ | 新 ✔ |
| --- | --- |
| 首页：`! RC003 按键隔离未就绪；麦克风音频仍可用，但前台应用可能收到录音键，详见「自检」。` | `! 未安装签名通道：遥控器连接时会拦截录音键（实体键盘的 F5 同时被拦截），断开后恢复。详见「自检」。` ✔ |
| 设置（状态行）：`尚未逐设备隔离：遥控器按键与实体键盘可能同时生效（签名通道为可选增强）` | `未安装签名通道：遥控器连接时会拦截录音键（实体键盘的 F5 同时被拦截），断开后恢复` ✔ |
| 设置（详情）：`未安装签名通道时，言灵**不拦截**来源未知的键：实体键盘保持原样…` ✗（**与事实相反** ✗ —— 在场时是会拦截的 ✔） | `未安装签名通道时，言灵按「遥控器是否在场」判断：在场时拦截录音键（实体键盘的 F5 也同时被拦截），非语音键的原生效果仍会透传。签名通道装上后改为按设备精确拦截。` ✔ |

三句都改成 **"现在怎样工作 / 什么时候会漏 / 要不要处理"** 的写法 ✔ —— 不再像"保护没开"✗。

### 四、验证

- **行为**：桥 self-test ✔、host self-test ✔、`validate` ✔；启动日志新增一行可核对 ✔（接线后实测 ✔）
- **文案**：三处页面文字判据实测 ✔；**目视**确认首页横幅完整无省略 ✗→✔（几何测不到文字省略 ✗，所以必须看图 ✔）
- 几何 `1280×840` **0 重叠 / 0 裁切** ✔、门禁已钉住（含"不得再写'不拦截来源未知的键'"✔）

## 2026-09-12 **把"收音电平"做进界面**（用户要求："确保收音稳定且优质" ✔）

### 先测量，再决定做什么（关键 ✔）

分析本机 **920 次**会话的采集日志（`vibe-mic-runtime.log` ✔）：

| 判据 | 数值 |
| --- | --- |
| 输出电平中位数 | **5%（`output_rms_pct`）** ✗ —— 识别舒适区约 **10–30%** ✔ |
| 低于 10% 的会话 | **841 / 918（92%）** ✗ |
| 峰值打到 100% 但均值 <5% | **311（34%）** ✗ → **杂音/碰触**，不是人声 ✔ |
| 峰值不高且输出 <10% | **546（59%）** ✗ → **声音太远/增益不足**（主因 ✔） |
| 完全没收到帧 | **55** ✗ |
| 链路质量 | `queue_drops=0` ✔、宿主判定 `HEALTHY` ✔、`maxGapMs=44–163` ✔ → **不是链路** ✗ |

**时间线**（这决定了责任归属 ✔）：从 **09-02** 起，逐小时中位电平就一直只有 **3–7%** ✗ —— 早于本会话所有改动 ✔，**不是这几轮造成的** ✗ ✔。

**结论（技术判断 ✔）**：软件侧**已经没有余量** ✗ —— `avg_gain` 已经在 2.4–5.8（约 4 倍 ✔），而 34% 的会话**已经削波** ✗，再提高增益只会更糟 ✗ ✔。真正的缺口是**声源电平** ✗（距离/朝向/电池 ✗ 与 Windows 输入电平 ✗），**并且用户看不见它** ✗。

### 本轮实现（不做音频链路改动 ✔）

1. **新增只读诊断脚本** ✔ `scripts/tests/Get-CaptureLevels.ps1`（提交 `da9bcd1` ✔）：打印最近 N 次会话的电平、增益、断档与判定 ✔，并给出按序的四条排查步骤 ✔
2. **把实测电平做进语音页** ✔：卡片底部（按钮之下、卡片内空白处 ✔）新增一行 ✔，读取 `sessionDir\vibe-mic-runtime.log` 的**最后一条** `REMOTE STREAM STOP` ✔（尾部读取 64 KB ✔，不整文件读 ✔），四种状态分别显示 ✔：
   - 无记录 → 「还没有记录 —— 完成一次听写后这里会显示实测值」✔
   - 没收到音频 → 「这一次没有收到音频 —— 检查遥控器是否已连接并唤醒」✔
   - **< 10%** → 「最近一次收音电平 X%（偏低，识别舒适区约 10–30%）：靠近到 10–20 cm、对准顶部麦克风孔并握稳；仍偏低时检查 Windows『CABLE Output → 属性 → 级别』是否为 100%」✔
   - ≥ 10% → 「X% · 处于识别舒适的区间」✔

**不碰的东西** ✔：`VibeMicAtvvCapture.exe`（冻结件、哈希固定 ✔）、`gain=1.0`/`autoLevel=true`（已验证稳定档 ✔）。
...
## 候选版 3 发版（2026-09-13 晚，用户确认"核心功能没有问题，可以安排发版" ✔）

**用户指令** ✔：① 锁定当前状态 ✔；② 固化参数并**发布到线上 GitHub** ✔；③ 同步更新使用教程、新版本功能介绍与截图 ✔。

**锁定与固化** ✔：
- 冻结清单新增 **§14 候选版 3 发版固化** ✔（品牌/标签/安装器清理、本轮三个真问题、生命周期验收三项、仍未验证项 ✔）。
- `frozen-parameters.json` 回填 **product=`Vibe Link`、tag=`v2.0.0-candidate.3`、commit、三个资产哈希、frozenAt** ✔，并新增门禁（此前**没有任何门禁读它** ✗）。
- 基线锁定文档 `docs/V2_0_BASELINE_LOCK_ZH.md` 更新到候选版 3 ✔。
- 打标签 **`v2.0.0-candidate.3`** 并推送 ✔，随后用 GitHub API 创建 Release 并上传三个附件 ✔。

**截图同步** ✔：用 `scripts/capture-ui-screenshots.ps1` 在**当前构建**上重抓六页 + 五步向导 + 对话框 ✔；实测确认语音页下拉框只有**微信输入法 / 网易八哥说 / 其他语音工具** ✔、品牌为 `Vibe Link V2.0.0` ✔、六页顺序正确 ✔。
- 发现教程里仍引用**旧的十一步向导截图** ✗（当前是五步）→ 已改为当前五张 ✔。
- 自检页当时显示 **"等待 2 项真实验证"**（遥控器按键监听需按一次实体键 ✗、VB-CABLE 可见性需点「优化可见性」 ✗）→ 发布页如实呈现，待用户操作后重抓 ✔。

**本轮修掉的真问题（都在发版前）** ✔：
1. **开机自启关不掉** ✗→✔（Run 值名写读不一致）✔
2. **干净克隆 / CI 无法构建** ✗→✔（VB-CABLE 包被 gitignore → 恢复脚本尽力获取 + 发布链改为可选）✔
3. **英文 Windows 下脚本无法解析** ✗→✔（缺 UTF-8 BOM：6 个 .ps1 + .iss + 48 个 .cs；冻结采集件源码有意例外 ✔）
4. **生命周期验收脚本自身两处 bug** ✗→✔（ANSI 读 UTF-8 造成中文比较假失败 ✗；清理非空目录触发 PS 5.1 `NullReferenceException` ✗）

**验收** ✔：`node scripts/validate.js` ✔、主机/桥/采集件自测 ✔、界面矩阵 **12/12** ✔、产物与身份校验 ✔、**生命周期三项全过** ✔（干净安装 / 未配置安装 / V1.5 升级）✔。
...
| 安装时注册表默认 `Ctrl + Shift + Alt + [` ✔ **但用户实测讯飞只允许设 F6** ✗ | `HKCU\Software\iFly Info Tek\iFlyIME → iFlyImeVoiceShiftHotKey` ✔（与安装包写入值逐字一致 ✔）；用户原话「快捷键只能设置为 f6」✔ → **应用改用 `f6`** ✔ |
...
### 新增：搜狗输入法 + 豆包输入法回归 + 网易八哥说改名（2026-09-13 深夜 ✔）

**用户要求** ✔：「帮我新增本机的搜狗输入语音输入法的功能。目前的快捷键配置是右侧的 Ctrl 激活」✔、「网易出的八哥说，帮我改名叫**网易八哥说**」✔、「如果可以，请帮我也适配**豆包输入法**」✔。

| 工具 | 应用内默认值 ✔ | 依据 ✔ |
| --- | --- | --- |
| **搜狗输入法** ✔ | `rightctrl` + **按住触发** ✔ | 用户自述「**右 Ctrl 激活**」✔ + 追问后确认「**按住右 Ctrl 说话、松开结束**」✔；本机 `搜狗输入法 16.6.0正式版`（`D:\SogouInput\16.6.0.4073`）✔、进程 `SGTool`/`SOGOU*`/`sogou_voice_assistant` 在跑 ✔ |
| **豆包输入法** ✔ | `alt+space` + 按住触发 ✔ | 早期适配时本机实测的语音快捷键 `Alt + 空格` ✔（项目记录：`C:\Program Files\DoubaoIME`，全局快捷键默认关闭 ✔） |
| **网易八哥说** ✔ | 仅**显示名**改动 ✔（`rightalt` / 单击切换不变 ✔） | 用户要求点名厂商 ✔ |

**保留在"已下线"里的只剩两个** ✔：Typeless / Windows 语音输入 ✔ —— `IsRetiredProviderValue` 现在**只剩这两个值** ✔，自测与门禁都钉住 ✔（豆包不再迁移 ✔）。

**新增能力** ✔：`FindInstalledToolLauncher()` ✔ —— 按卸载表**显示名**找启动器 ✔，并在 `InstallLocation` / `DisplayIcon` 目录**及其一级版本子目录**里找候选 exe ✔（讯飞 `D:\iFlyIME\3.0.1750`、搜狗 `D:\SogouInput\16.6.0.4073` 这类"安装目录带版本号"都能命中 ✔）；搜狗 TSF 身份 `{E7EA138E-…}` + `{E7EA138F-…}` ✔ 已写入输入法识别表 ✔。

**证据** ✔：主机自测**穷举六个工具的默认快捷键两两不同** ✔ + 搜狗/豆包接线断言 ✔ + 引擎识别断言 ✔；`validate.js` 门禁同步（下拉框六项、索引映射、退役集合、HUD 名称）✔；发布链矩阵/载荷/安装器按常规复核 ✔。
...
## 2026-09-13 · 候选版 3 发布与发布页修复

### 已发布（线上）

- GitHub 预发布 `v2.0.0-candidate.3`：标签指向 `d7d33e1`，`prerelease=true`（Latest 仍是 v1.5.0），三个资产：`VibeFlow-Setup.exe`、`Vibe-Flow-Windows-x64.zip`、`SHA256SUMS.txt`。
- 资产核对：重新从公开下载地址取回三个文件，SHA-256 与 `frozen-parameters.json` 的 `releaseAssets` **逐字节一致** ✔；发布页正文 8 张图片与 8 个文档链接全部 200 ✔（图片逐字节等于标签下的本地副本 ✔）。
- 首次发布暴露两个同源缺陷，都已修复：发布脚本没有 BOM，Windows PowerShell 5.1 按 ANSI 读取，**发布名变成乱码**、正文被 GitHub 直接拒绝。现在发布名从 `frozen-parameters.json` 读、正文按 UTF-8 字节解码；发布脚本改为幂等（存在则 `PATCH`，缺资才上传）。

### 发布页与截图

- 发布页截图行去重：末行第二个格子原本指向 `03-shortcuts-screenshot.png`，它与 `03-shortcuts.png` **字节相同**，于是同一张图在一行里出现两次。改为「语音工具：三选一」下拉截图（`06-transcription-tools.png`），并核对标签下该图与本地一致 ✔。
- 新增三条门禁：正文引用的截图必须存在；**同一小节内**截图不得重复（跨小节复用允许，例如首屏总览与「5 分钟上手」各用一次）；`capture-ui-screenshots.ps1` 用文案匹配的每个标签，必须仍能在产品源码里找到。第二条曾把「首屏总览 + 上手章节」两处复用也判为重复，因此按小节判定。
- 门禁负向对照：把同一张图放进同一小节 → 报错并指出小节名 ✔；把 `录制键盘快捷键` 改一个字 → 报错并指出标签 ✔。
- `scripts/capture-ui-screenshots.ps1` 修因：它一直在找 V1.5 时代的动作框文案 `保持上方向（推荐）`，V2.0 已改名，`Test-ChildText` 判定失败后**静默跳过**，所以 `07/08` 自 2026-09-03 起从未被重拍。现在改为匹配当前动作框 `上方向`、对话框标题前缀 `配置 `，找不到就**报错**而不是跳过；新增 `-DialogsOnly` 只重拍对话框。
- 重拍 `07-shortcut-actions.png`、`08-shortcut-recorder.png`、`09-smart-profile-apps.png`（出货构建；动作选择器标题为「配置 上键短按」）✔。README 引用的两张对话框截图因此不再是 V1.5 画面。

### CI 生命周期

- CI 上「干净安装」「未配置安装」两步**首次通过**（此前因无 BOM 立即失败）✔；「V1.5 升级」步仍失败，而**本机同一序列通过**。
- 已排除迁移助手本身：按安装器真实引用方式运行 `VibeFlow.exe --installer-config-migrate "<旧目录>" "<用户数据目录>"`，本机对同一份 fixture（含已退役的 `typeless`）**exit 0 且逐字复制** ✔（探针第一次失败是因为我自己多写了一个尾随反斜杠，把两个参数并成了一个，宿主用 exit 12 拒绝——那正是历史上记录过的引号缺陷）。
- 已在安装器与测试里加日志：安装器记录「注册表给出的上一安装目录」「迁移时旧文件是否存在」「迁移助手退出码」，测试传 `/LOG` 并在投影不一致时打印这些行。**本阶段不写成已修复**，下一次 CI 运行据此定位。

### V1.5 升级步在 CI 上失败 —— 已定位并加守卫（待下一次 CI 运行确认）

- 安装器日志给出的事实：注册表给出的上一安装目录**正确** ✔、迁移时旧文件**存在** ✔、迁移助手**退出码 0** ✔ —— 也就是说安装器按契约做完了，但目标目录里躺着的仍是**默认配置**。
- 真因（从 V1.5 产物本身读出，不是推测）：`https://raw.githubusercontent.com/richlearntodo-debug/vibe-flow/v1.5.0/installer/VibeFlow.iss` 的 `[Run]` 是
  `Filename: "{app}\{#MyAppExeName}"; … Flags: nowait postinstall` —— **没有 `skipifsilent`**。所以**静默安装 V1.5 也会把 V1.5 应用启动起来**；它按 V1.5 的规矩把配置写在**自己的安装目录**里，启动后把测试刚写下的 fixture **覆盖成 V1.5 的默认配置**。候选版安装器的迁移只检查“旧文件是否存在”，于是**忠实地搬走了一份被换过的文件** ✔ → 断言必然不等。
- 本机为什么通过 ✔：本机有一个**在跑的候选版实例**（`Local\VibeMic` 互斥体），V1.5 应用启动后立刻退出，因此 fixture 没被覆盖 —— 这是"本机过、CI 不过"的完整解释，而不是"环境玄学"。
- 已加守卫 ✔：安装前停掉所有 Vibe Link 进程（逐个打印路径），并对 fixture 做 **SHA-256 自证**（最多 6 轮：再停一次进程 → 比对 → 不符就重写），若仍被改写就**报错并说明会测错文件**；投影不一致时还会打印「fixture 是否仍完好」。
- **本轮不写成已修复**：仍需一次 CI 运行确认（守卫同时会打印它到底停掉了几个进程、路径是什么，这本身就是证据）。

#### 真因（由 CI 日志时间戳定案，2026-09-13 11:59）

- 目标配置文件的**最后写入时间 11:59:19.467**，而候选版安装器的 `InitializeWizard` 日志是 **11:59:20.153**、迁移调用是 **11:59:20.487** —— 也就是说**迁移之前目标文件就已经存在了**，迁移按设计跳过（"永不覆盖已有的用户配置"），随后 `TryReadUserConfigCandidate` 判定成功 → **exit 0** ✔，测试于是拿"V1.5 应用写下的默认配置"去和 fixture 比对。
