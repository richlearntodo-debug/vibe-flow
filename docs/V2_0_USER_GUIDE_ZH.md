...
选择一个语音工具（微信输入法 / 网易八哥说 / 讯飞语音输入法 / 搜狗输入法 / 豆包输入法 / 其他语音工具），并按工具自己的说明设置快捷键和触发方式。Vibe Flow 两边的快捷键必须完全一致。选择讯飞语音输入法时，请在讯飞输入法「设置 → 语音」把语音快捷键设为 `F6`，并选择「长按说话」；选择搜狗输入法时，请在搜狗自己的设置里把语音输入快捷键设为右 Ctrl 并保持「按住说话」；选择豆包输入法时，请确认它的语音快捷键是 `Alt + 空格`、保持按住说话，并让言灵保存的快捷键和触发方式与它完全一致。
...
当前候选版提供六个可选语音工具，顺序与「语音」页一致：

1. **微信输入法**（默认，`Ctrl + Win`，单击切换）；
2. **网易八哥说**（右 Alt，单击切换）；
3. **讯飞语音输入法**（本候选版新增支持，`F6`，按住触发）；
4. **搜狗输入法**（本候选版新增支持，右 Ctrl，按住触发）；
5. **豆包输入法**（本候选版重新提供，`Alt + 空格`，按住触发；支持但未验证）；
6. **其他语音工具**（自定义，默认右 Shift，单击切换）。

无论选哪一个，应用里保存的快捷键和触发方式都必须与工具自己配置的完全一致，否则按下录音键不会启动听写。

免驱动模式（未检测到 VB-CABLE、没有采集进程）下，主机能自行唤起网易八哥说、讯飞语音输入法、搜狗输入法和自定义工具；微信输入法需要它自己的语音面板，因此仍需要 VB-CABLE。

### 微信输入法

微信输入法仍是默认工具，稳定档案是 `Ctrl + Win` 单击切换。它是第三方工具：部分版本会先把识别结果放入剪贴板，而不是直接向当前控件注入文字。Vibe Flow 不读取剪贴板文字，也不能保证它在所有 ChatGPT、浏览器或提权窗口中直写。
...
4. 先不要依赖自动粘贴：网易八哥说是否把文字直接写进输入框**尚未在本机实测**，所以言灵**不会**为它派发 `Ctrl+V`。如果实测发现它其实把结果放进剪贴板，需要在语音页改用「其他语音工具」并手动粘贴，或等待我们对它的投递方式做实测后适配。
5. 言灵只负责派发触发动作与维持录音，**不读取**网易八哥说的转写内容。

### 讯飞语音输入法

讯飞语音输入法（iFlytek，本机安装在 `D:\iFlyIME\3.0.1750`）是本候选版新增支持的工具，默认快捷键是 `F6`，触发方式是**按住触发**。

1. 在「语音」页把「转写工具」选成 **讯飞语音输入法**；默认快捷键会填 `F6`，触发方式默认是「按住触发」。
2. **在讯飞输入法「设置 → 语音」里确认它的语音快捷键就是 `F6`**（讯飞只允许设成 F6）。如果它在客户端里用的是别的组合，请把语音页的快捷键改成**完全一致**的那个。
3. 触发方式选择「长按说话」，与应用里的「按住触发」对应：按住遥控器录音键开始，松开结束。
4. 讯飞语音输入法要求它自己是当前输入法时语音快捷键才生效。先在目标输入框里切到讯飞输入法，用键盘按一次上面的组合确认能启动与结束，再用遥控器录音键测试。
5. 言灵只负责派发触发动作与维持录音，**不读取**讯飞的转写内容；文字由讯飞自己上屏，通常不经过剪贴板。

### 搜狗输入法

搜狗输入法（本机安装在 `D:\SogouInput\16.6.0.4073`）是本候选版新增支持的工具，默认快捷键是**右 Ctrl**（`rightctrl`），触发方式是**按住触发**。

1. 在「语音」页把「转写工具」选成 **搜狗输入法**；默认快捷键会填 **右 Ctrl**，触发方式默认是「按住触发」。
2. **在搜狗输入法自己的设置里确认它的语音输入快捷键就是右 Ctrl**。用户必须让搜狗自己的语音/按键设置与言灵里保存的快捷键保持相等：如果搜狗里用的是别的组合，请把语音页的快捷键改成**完全一致**的那个（两边不一致时，按键按下不会启动听写）。
3. 保持「按住说话」的用法，与应用里的「按住触发」对应：按住遥控器录音键开始，松开结束。先在键盘上按住右 Ctrl 确认搜狗能启动与结束，再用遥控器录音键测试。
4. 言灵只负责派发触发动作与维持录音，**不读取**搜狗的转写内容。

### 豆包输入法（支持但未验证）

豆包输入法在本候选版重新提供（它此前曾被下线，现按用户要求恢复），默认快捷键是 `Alt + 空格`，触发方式是**按住触发**。它属于**支持但未验证**的工具，不是已验证路径。

1. 在「语音」页把「转写工具」选成 **豆包输入法**；默认快捷键会填 `Alt + 空格`，触发方式默认是「按住触发」。
2. **在豆包输入法自己的设置里确认它的语音快捷键就是 `Alt + 空格`**，并保持按住说话的用法；言灵里保存的快捷键和触发方式必须与它完全一致。
3. 两项历史风险仍未验证，请不要按“已经能用”来预期：①豆包的语音面板可能忽略模拟按键，遥控器可能唤不起它；②它的麦克风列表可能不提供虚拟声卡 `CABLE Output`，隔离音频通道可能选不到它。若任意一点成立，改用网易八哥说、讯飞语音输入法或搜狗输入法。
...
| 现象 | 先检查 | 不要做什么 |
| --- | --- | --- |
| 文字进了剪贴板 | 语音工具是否为微信输入法；目标是否仍有光标；改用讯飞语音输入法对照 | 不要让 Vibe Flow 读取剪贴板文字回填，不要连续按录音键 |
